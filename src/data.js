export const RAW_TRIPS = [
  {
    "name": "Braj, Kashi & Kalinga",
    "cities": [
      {
        "code": "gurugram",
        "dur": "2 Weeks"
      },
      {
        "code": "mathura",
        "dur": "2 Days"
      },
      {
        "code": "agra",
        "dur": "1 Day"
      },
      {
        "code": "varanasi",
        "dur": "3 Days"
      },
      {
        "code": "kolkata",
        "dur": "2 Days"
      },
      {
        "code": "puri-konark",
        "dur": "2 Days"
      },
      {
        "code": "noida",
        "dur": "2 Days"
      }
    ],
    "distanceKm": 3041
  },
  {
    "name": "Mewar & Saurashtra",
    "cities": [
      {
        "code": "delhi",
        "dur": "2 Day"
      },
      {
        "code": "jaipur",
        "dur": "3 Days"
      },
      {
        "code": "chittorgarh",
        "dur": "1 Day"
      },
      {
        "code": "udaipur",
        "dur": "3 Days"
      },
      {
        "code": "kumbhalgarh",
        "dur": "1 Day"
      },
      {
        "code": "ahmedabad-gandhinagar",
        "dur": "3 Days"
      },
      {
        "code": "kevadiya",
        "dur": "1 Day"
      },
      {
        "code": "surat",
        "dur": "1 Day"
      },
      {
        "code": "vadodara",
        "dur": "1 Day"
      },
      {
        "code": "somnath",
        "dur": "1 Day"
      },
      {
        "code": "dwarka",
        "dur": "2 Days"
      },
      {
        "code": "rajkot",
        "dur": "1 Day"
      },
      {
        "code": "delhi",
        "dur": "2 Days"
      }
    ],
    "distanceKm": 3024
  },
  {
    "name": "Punjab, Jammu & Kashmir",
    "cities": [
      {
        "code": "ludhiana",
        "dur": "1 Month"
      },
      {
        "code": "jalandhar",
        "dur": "1 Day"
      },
      {
        "code": "amritsar",
        "dur": "2 Days"
      },
      {
        "code": "pathankot",
        "dur": "0.5 Day"
      },
      {
        "code": "katra",
        "dur": "2 Days"
      },
      {
        "code": "jammu",
        "dur": "1 Day"
      },
      {
        "code": "banihar-qazigund",
        "dur": "1 Day"
      },
      {
        "code": "srinagar",
        "dur": "3 Days"
      },
      {
        "code": "gulmarg",
        "dur": "1 Day"
      },
      {
        "code": "kargil",
        "dur": "1 Day"
      },
      {
        "code": "leh",
        "dur": "3 Days"
      },
      {
        "code": "keylong",
        "dur": "1 Day"
      },
      {
        "code": "manali",
        "dur": "3 Days"
      },
      {
        "code": "kullu",
        "dur": "1 Day"
      },
      {
        "code": "chandigarh",
        "dur": "3 Days"
      },
      {
        "code": "ludhiana",
        "dur": "2 Months"
      }
    ],
    "distanceKm": 1367
  },
  {
    "name": "Thar, Deccan & Alleppey",
    "cities": [
      {
        "code": "gurugram",
        "dur": "1 Week"
      },
      {
        "code": "bikaner",
        "dur": "1 Day"
      },
      {
        "code": "sam",
        "dur": "2 Days"
      },
      {
        "code": "jaisalmer",
        "dur": "4 Days"
      },
      {
        "code": "jodhpur",
        "dur": "4 Days"
      },
      {
        "code": "ajmer-pushkar",
        "dur": "2 Days"
      },
      {
        "code": "ahmedabad-gandhinagar",
        "dur": "1 Week"
      },
      {
        "code": "indore",
        "dur": "1 Day"
      },
      {
        "code": "omkareshwar",
        "dur": "1 Day"
      },
      {
        "code": "nashik",
        "dur": "2 Days"
      },
      {
        "code": "shirdi",
        "dur": "2 Days"
      },
      {
        "code": "sambhajinagar",
        "dur": "1 Week"
      },
      {
        "code": "pune",
        "dur": "1 Week"
      },
      {
        "code": "mumbai",
        "dur": "2 Weeks"
      },
      {
        "code": "goa",
        "dur": "1 Week"
      },
      {
        "code": "kochi",
        "dur": "3 Days"
      },
      {
        "code": "allepey",
        "dur": "2 Day"
      },
      {
        "code": "varkala",
        "dur": "1 Week"
      },
      {
        "code": "trivandrum",
        "dur": "1 Week"
      },
      {
        "code": "vagamon",
        "dur": "1 Day"
      },
      {
        "code": "coimbatore",
        "dur": "1 Week"
      },
      {
        "code": "ooty",
        "dur": "4 Days"
      },
      {
        "code": "mysore",
        "dur": "1 Week"
      },
      {
        "code": "bengaluru",
        "dur": "2 Weeks"
      },
      {
        "code": "gurugram",
        "dur": "4 Weeks"
      }
    ],
    "distanceKm": 6305
  },
  {
    "name": "Wayanad, Northeast & Sikkim",
    "cities": [
      {
        "code": "gurugram",
        "dur": "1 Week"
      },
      {
        "code": "bengaluru",
        "dur": "3 Days"
      },
      {
        "code": "wayanad",
        "dur": "1 Week"
      },
      {
        "code": "bengaluru",
        "dur": "3 Days"
      },
      {
        "code": "rameshwaram",
        "dur": "2 Days"
      },
      {
        "code": "madurai",
        "dur": "3 Days"
      },
      {
        "code": "coimbatore",
        "dur": "1 Week"
      },
      {
        "code": "guwahati",
        "dur": "3 Days"
      },
      {
        "code": "shillong",
        "dur": "3 Days"
      },
      {
        "code": "dawki",
        "dur": "2 Days"
      },
      {
        "code": "silchar",
        "dur": "2 Days"
      },
      {
        "code": "dimapur",
        "dur": "3 Days"
      },
      {
        "code": "kohima",
        "dur": "1 Week"
      },
      {
        "code": "tinsukia",
        "dur": "2 Days"
      },
      {
        "code": "itanagar",
        "dur": "1 Day"
      },
      {
        "code": "tawang",
        "dur": "1 Week"
      },
      {
        "code": "guwahati",
        "dur": "2 Days"
      },
      {
        "code": "darjeeling",
        "dur": "5 Days"
      },
      {
        "code": "pelling",
        "dur": "4 Days"
      },
      {
        "code": "gangtok",
        "dur": "1 Week"
      },
      {
        "code": "siliguri",
        "dur": "1 Day"
      },
      {
        "code": "kolkata",
        "dur": "1 Week"
      },
      {
        "code": "varanasi",
        "dur": "1 Week"
      },
      {
        "code": "gurugram",
        "dur": "3 Weeks"
      },
      {
        "code": "noida",
        "dur": "1 Month"
      }
    ],
    "distanceKm": 8646
  },
  {
    "name": "Deccan, Rayalaseema & Kashmir",
    "cities": [
      {
        "code": "gurugram",
        "dur": "1 Week"
      },
      {
        "code": "bengaluru",
        "dur": "2 Weeks"
      },
      {
        "code": "kanchipuram",
        "dur": "2 Days"
      },
      {
        "code": "tirupati",
        "dur": "1 Day"
      },
      {
        "code": "srisailam",
        "dur": "1 Day"
      },
      {
        "code": "hyderabad",
        "dur": "1 Week"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "shivamogga",
        "dur": "3 Days"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "pondicherry",
        "dur": "5 Days"
      },
      {
        "code": "chennai",
        "dur": "3 Days"
      },
      {
        "code": "srinagar",
        "dur": "1 Week"
      },
      {
        "code": "sonmarg",
        "dur": "1 Day"
      },
      {
        "code": "baltal",
        "dur": "1 Day"
      },
      {
        "code": "srinagar",
        "dur": "1 Day"
      },
      {
        "code": "banihal-udhampur",
        "dur": "1 Day"
      },
      {
        "code": "ludhiana",
        "dur": "1 Week"
      },
      {
        "code": "gurugram",
        "dur": "2 Weeks"
      }
    ],
    "distanceKm": 7065
  },
  {
    "name": "Bundelkhand, Puravanchal & Malabar",
    "cities": [
      {
        "code": "gurugram",
        "dur": "1 Week"
      },
      {
        "code": "orchha",
        "dur": "2 Days"
      },
      {
        "code": "jhansi",
        "dur": "1 Day"
      },
      {
        "code": "khajurao",
        "dur": "2 Days"
      },
      {
        "code": "prayagraj",
        "dur": "2 Days"
      },
      {
        "code": "ayodhya",
        "dur": "2 Days"
      },
      {
        "code": "lukhnow",
        "dur": "1 Day"
      },
      {
        "code": "varanasi",
        "dur": "2 Days"
      },
      {
        "code": "bodh-gaya",
        "dur": "2 Days"
      },
      {
        "code": "deoghar",
        "dur": "1 Day"
      },
      {
        "code": "kolkata",
        "dur": "2 Weeks"
      },
      {
        "code": "digha",
        "dur": "1 Day"
      },
      {
        "code": "kharagpur",
        "dur": "1 Day"
      },
      {
        "code": "cuttak",
        "dur": "1 Day"
      },
      {
        "code": "vishakhapatnam",
        "dur": "1 Week"
      },
      {
        "code": "araku",
        "dur": "1 Day"
      },
      {
        "code": "hyderabad",
        "dur": "1 Week"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "mysore",
        "dur": "2 Days"
      },
      {
        "code": "coorg",
        "dur": "2 Days"
      },
      {
        "code": "chikkmanglore",
        "dur": "3 Days"
      },
      {
        "code": "hampi",
        "dur": "2 Days"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "mudurai",
        "dur": "1 Day"
      },
      {
        "code": "kodaikannal",
        "dur": "1 Week"
      },
      {
        "code": "poombarai",
        "dur": "1 Day"
      },
      {
        "code": "theni",
        "dur": "1 Day"
      },
      {
        "code": "munnar",
        "dur": "3 Days"
      },
      {
        "code": "trivandrum",
        "dur": "1 Day"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "thrissur",
        "dur": "1 Day"
      },
      {
        "code": "palakkad",
        "dur": "1 Day"
      },
      {
        "code": "coimbatore",
        "dur": "1 Week"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "gurugram",
        "dur": "1 Week"
      }
    ],
    "distanceKm": 8350
  },
  {
    "name": "Vidarbha, Konkan & Kutch",
    "cities": [
      {
        "code": "delhi",
        "dur": "1 Week"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "gwalior",
        "dur": "3 Days"
      },
      {
        "code": "nagpur",
        "dur": "4 Days"
      },
      {
        "code": "hyderabad",
        "dur": "2 Week"
      },
      {
        "code": "bengaluru",
        "dur": "3 Weeks"
      },
      {
        "code": "gokarna",
        "dur": "1 Week"
      },
      {
        "code": "goa",
        "dur": "2 Weeks"
      },
      {
        "code": "ahmedabad-gandhinagar",
        "dur": "1 Week"
      },
      {
        "code": "bhuj",
        "dur": "2 Days"
      },
      {
        "code": "kutchh",
        "dur": "5 Days"
      },
      {
        "code": "surat",
        "dur": "2 Days"
      },
      {
        "code": "mumbai",
        "dur": "1 Month"
      },
      {
        "code": "gurugram",
        "dur": "4 Months"
      }
    ],
    "distanceKm": 8133
  },
  {
    "name": "Coastal Karnataka, MP & Himachal",
    "cities": [
      {
        "code": "gurugram",
        "dur": "1 Week"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "delhi",
        "dur": "2 Days"
      },
      {
        "code": "bhopal",
        "dur": "2 Days"
      },
      {
        "code": "hyderabad",
        "dur": "1 Week"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "gokarna",
        "dur": "1 Week"
      },
      {
        "code": "udupi",
        "dur": "2 Days"
      },
      {
        "code": "manglore",
        "dur": "2 Days"
      },
      {
        "code": "kannur",
        "dur": "1 Week"
      },
      {
        "code": "kozhikode",
        "dur": "2 Days"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "gurugram",
        "dur": "2 Weeks"
      },
      {
        "code": "amritsar",
        "dur": "3 Days"
      },
      {
        "code": "dalhouisie",
        "dur": "3 Days"
      },
      {
        "code": "dharamshala-mcleond-gang",
        "dur": "1 Week"
      },
      {
        "code": "bir",
        "dur": "5 Days"
      },
      {
        "code": "gurugram",
        "dur": "2 Weeks"
      }
    ],
    "distanceKm": 9070
  },
  {
    "name": "Coromandel, Kochi & Bengal",
    "cities": [
      {
        "code": "gurugram",
        "dur": "1 Week"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "wayanad-vyithri",
        "dur": "3 Days"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "chennai",
        "dur": "1 Day"
      },
      {
        "code": "mahabalipuram",
        "dur": "1 Day"
      },
      {
        "code": "thanjavur",
        "dur": "2 Days"
      },
      {
        "code": "tiruchirappalli",
        "dur": "2 Days"
      },
      {
        "code": "kanyakumari",
        "dur": "2 Days"
      },
      {
        "code": "kochi",
        "dur": "1 Week"
      },
      {
        "code": "kozhikode",
        "dur": "2 Days"
      },
      {
        "code": "bengaluru",
        "dur": "1 Week"
      },
      {
        "code": "kolkata",
        "dur": "2 Months"
      },
      {
        "code": "mayapur",
        "dur": "1 Day"
      },
      {
        "code": "mirik",
        "dur": "1 Week"
      },
      {
        "code": "kurseong",
        "dur": "2 Days"
      },
      {
        "code": "ghoom",
        "dur": "2 Days"
      },
      {
        "code": "darjeeling",
        "dur": "2 Days"
      },
      {
        "code": "kalimpong",
        "dur": "1 Week"
      },
      {
        "code": "kolkata",
        "dur": "3 Days"
      },
      {
        "code": "patna",
        "dur": "1 Week"
      },
      {
        "code": "gurugram",
        "dur": "1 Month"
      }
    ],
    "distanceKm": 7698
  }
];

export const CITIES = {
  "gurugram": {
    "city": "Gurugram",
    "lat": 28.4601,
    "lng": 77.02635,
    "state": "haryana",
    "region": "northern",
    "category": [
      "metro",
      "base_camp"
    ],
    "must_visit": [
      "Cyber Hub",
      "Kingdom of Dreams",
      "Ambience Mall",
      "Sultanpur Bird Sanctuary",
      "Leisure Valley Park"
    ],
    "must_try": [
      "Biryani at Cyber Hub restaurants",
      "Desi ghee parathas at Old Gurugram dhabas",
      "Craft beer at microbreweries",
      "Street food at Sadar Bazar",
      "Gurugram food truck trail",
      "Corporate networking walkabout in Sector 29"
    ],
    "accommodation": [
      "gostops",
      "bloom"
    ],
    "mode_of_travel": [
      "metro",
      "ola/uber",
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "More of a layover city than a destination — the food scene around Cyber Hub genuinely surprised me.",
      // "Traffic is brutal after 6pm, plan your evenings inside Cyber Hub or near your hotel.",
      // "If you are working remotely, the co-working culture here is top-notch — cafes everywhere."
    // ]
  },
  "mathura": {
    "city": "Mathura",
    "lat": 27.4924,
    "lng": 77.6737,
    "state": "uttar_pradesh",
    "region": "northern",
    "category": [
      "pilgrimage"
    ],
    "must_visit": [
      "Krishna Janmabhoomi Temple",
      "Dwarkadhish Temple",
      "Vishram Ghat",
      "Kusum Sarovar",
      "Govardhan Hill"
    ],
    "must_try": [
      "Peda from Brijwasi sweets",
      "Bedai and jalebi breakfast",
      "Evening aarti at Vishram Ghat",
      "Holi celebration (if in season)",
      "Parikrama of Govardhan",
      "Rabri and malpua"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "The Vishram Ghat evening aarti is genuinely moving — smaller and more intimate than Varanasi.",
      // "Holi here is the real deal — if you can handle the crowd, nothing else comes close.",
      // "The peda here is legitimately different from anywhere else, stock up before leaving."
    // ]
  },
  "agra": {
    "city": "Agra",
    "lat": 27.1767,
    "lng": 78.0081,
    "state": "uttar_pradesh",
    "region": "northern",
    "category": [
      "forts_palaces",
      "monuments_museums"
    ],
    "must_visit": [
      "Taj Mahal",
      "Agra Fort",
      "Fatehpur Sikri",
      "Itmad-ud-Daula Tomb",
      "Mehtab Bagh"
    ],
    "must_try": [
      "Petha from Panchhi Petha",
      "Bedai and aloo sabzi breakfast",
      "Mughlai biryani at local restaurants",
      "Sunrise view of Taj from Mehtab Bagh",
      "Dalmoth and namkeen from street vendors"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "The Taj at sunrise from Mehtab Bagh across the Yamuna is far better than fighting the main gate crowd.",
      // "Fatehpur Sikri is criminally underrated — go on a weekday for a near-empty experience.",
      // "Agra is a 1-night city max — do it efficiently and move on."
    // ]
  },
  "varanasi": {
    "city": "Varanasi",
    "lat": 25.3176,
    "lng": 82.9739,
    "state": "uttar_pradesh",
    "region": "northern",
    "category": [
      "pilgrimage",
      "monuments_museums",
      "backpacker"
    ],
    "must_visit": [
      "Dashashwamedh Ghat",
      "Kashi Vishwanath Temple",
      "Manikarnika Ghat",
      "Sarnath",
      "Assi Ghat"
    ],
    "must_try": [
      "Banarasi chai in kullad",
      "Malaiyo in winter mornings",
      "Thandai at Blue Lassi shop",
      "Tamatar chaat at Kachori Gali",
      "Ganga aarti at Dashashwamedh",
      "Boat ride at dawn",
      "Banarasi paan"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "moustache"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Varanasi doesn't reveal itself on day one — you need at least two full days to stop feeling overwhelmed and start feeling it.",
      // "The pre-dawn boat ride is non-negotiable, no matter how cold it gets.",
      // "Manikarnika is confronting and profound — approach with respect, no camera."
    // ]
  },
  "kolkata": {
    "city": "Kolkata",
    "lat": 22.5726,
    "lng": 88.3639,
    "state": "west_bengal",
    "region": "eastern",
    "category": [
      "metro",
      "monuments_museums",
      "backpacker",
      "base_camp"
    ],
    "must_visit": [
      "Victoria Memorial",
      "Howrah Bridge",
      "Dakshineswar Kali Temple",
      "Park Street",
      "Kumartuli",
      "Indian Museum"
    ],
    "must_try": [
      "Kathi roll from Nizam's",
      "Mishti doi & sandesh from local mishti shops",
      "Puchka at College Street",
      "Kosha mangsho",
      "Ride a hand-pulled rickshaw in Shyambazar",
      "Evening adda at a chai stall",
      "Book-hunting at College Street"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "metro",
      "city_bus",
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 4,
    // "notes": [
      // "Kolkata hits different in winter — the city slows down just enough to let you breathe it in.",
      // "Every lane has a story and every stranger has an opinion — embrace it.",
      // "The food here will ruin you for everywhere else, and that's perfectly fine."
    // ]
  },
  "puri-konark": {
    "city": "Puri & Konark",
    "lat": 19.8135,
    "lng": 85.8312,
    "state": "odisha",
    "region": "eastern",
    "category": [
      "pilgrimage",
      "coastal",
      "monuments_museums"
    ],
    "must_visit": [
      "Jagannath Temple Puri",
      "Puri Beach",
      "Konark Sun Temple",
      "Chandrabhaga Beach",
      "Raghurajpur Craft Village",
      "Chilika Lake (day trip)"
    ],
    "must_try": [
      "Mahaprasad from Ananda Bazaar",
      "Chhena poda",
      "Dalma dal",
      "Sea-side macha (fish) fry at Puri beach shacks",
      "Watch Rath Yatra procession route if timing allows",
      "Sunrise at Konark"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "auto",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Konark at sunrise with almost no one around is one of those images you carry forever.",
      // "Mahaprasad at Ananda Bazaar feels sacred even if you're not religious — eat it sitting on the floor.",
      // "Puri beach is noisy and touristy but the sunset still delivers, every single time."
    // ]
  },
  "noida": {
    "city": "Noida",
    "lat": 28.58,
    "lng": 77.33,
    "state": "uttar_pradesh",
    "region": "northern",
    "category": [
      "metro",
      "base_camp"
    ],
    "must_visit": [
      "Worlds of Wonder",
      "Botanical Garden",
      "Okhla Bird Sanctuary",
      "Great India Place Mall",
      "Sector 18 Market"
    ],
    "must_try": [
      "UP-style kachori sabzi",
      "Kulfi from Sector 18 stalls",
      "Chaat at Atta Market",
      "Street momos near Film City",
      "Imarti and jalebi at Old Noida sweet shops"
    ],
    "accommodation": [
      "zostel",
      "bloom"
    ],
    "mode_of_travel": [
      "metro",
      "ola/uber",
      "auto",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Noida is the quieter, more planned twin of Delhi — useful base for exploring UP heritage sites.",
      // "The metro connectivity to Delhi is genuinely seamless, best way to get around.",
      // "Sector 18 after dark has decent street food energy but nothing particularly unique."
    // ]
  },
  "delhi": {
    "city": "Delhi",
    "lat": 28.65195,
    "lng": 77.23149,
    "state": "delhi",
    "region": "northern",
    "category": [
      "metro",
      "forts_palaces",
      "monuments_museums",
      "pilgrimage",
      "base_camp"
    ],
    "must_visit": [
      "Red Fort",
      "Qutub Minar",
      "Humayun's Tomb",
      "Jama Masjid & Chandni Chowk",
      "Lodhi Garden",
      "Akshardham Temple"
    ],
    "must_try": [
      "Parathe wali gali parathas",
      "Nihari at Karim's Jama Masjid",
      "Chole bhature at Sitaram Diwan Chand",
      "Jalebi from Old Delhi halwais",
      "Daulat ki chaat in winter",
      "Cycling in Lodhi Garden at dawn",
      "Walk through Mehrauli Archaeological Park"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "the_hosteller",
      "moustache",
      "bloom"
    ],
    "mode_of_travel": [
      "metro",
      "ola/uber",
      "auto",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Old Delhi is sensory overload in the best possible way — Chandni Chowk at 7am before the crowds is magic.",
      // "The sheer density of Mughal and pre-Mughal monuments here rivals any world capital.",
      // "Pollution in November-January is real — carry a mask if you plan outdoor sightseeing."
    // ]
  },
  "jaipur": {
    "city": "Jaipur",
    "lat": 26.91962,
    "lng": 75.78781,
    "state": "rajasthan",
    "region": "northern",
    "category": [
      "metro",
      "forts_palaces",
      "monuments_museums"
    ],
    "must_visit": [
      "Amber Fort",
      "Hawa Mahal",
      "City Palace",
      "Jantar Mantar",
      "Nahargarh Fort",
      "Jal Mahal"
    ],
    "must_try": [
      "Dal Baati Churma",
      "Ghewar",
      "Laal Maas",
      "Pyaaz Kachori",
      "Chai at Lassiwala",
      "Elephant ride at Amer",
      "Shopping on Johari Bazaar"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "the_hosteller",
      "moustache",
      "madpackers",
      "bloom"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "The pink city really earns its name at dusk when the sandstone turns gold — Amber Fort at sunrise is worth the early wake-up call.",
      // "Johari Bazaar is absolute chaos and I loved every second of it — bargaining for blue pottery and silver jewellery is half the fun.",
      // "Auto-rickshaw drivers will try every trick in the book — settle the price before you get in, always."
    // ]
  },
  "chittorgarh": {
    "city": "Chittorgarh",
    "lat": 24.88963,
    "lng": 74.62403,
    "state": "rajasthan",
    "region": "northern",
    "category": [
      "forts_palaces",
      "monuments_museums"
    ],
    "must_visit": [
      "Chittorgarh Fort",
      "Vijay Stambha",
      "Kirti Stambha",
      "Padmini Palace",
      "Meera Temple"
    ],
    "must_try": [
      "Dal Baati Churma",
      "Mawa Kachori",
      "Cycling around the fort",
      "Sunset at Vijay Stambha",
      "Local bazaar walk"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "The fort is massive — bigger than most Indian cities — and the weight of history hits you the moment you pass through the first gate.",
      // "Most tourists skip Chittorgarh for Udaipur but honestly the stories of Rani Padmini and Maharana Pratap make this the more emotionally resonant stop.",
      // "Hire a local guide, not a tourist agency one — the difference in storytelling quality is night and day."
    // ]
  },
  "udaipur": {
    "city": "Udaipur",
    "lat": 24.5854,
    "lng": 73.7125,
    "state": "rajasthan",
    "region": "northern",
    "category": [
      "forts_palaces"
    ],
    "must_visit": [
      "Lake Pichola",
      "City Palace",
      "Jag Mandir",
      "Saheliyon ki Bari",
      "Fateh Sagar Lake",
      "Jagdish Temple"
    ],
    "must_try": [
      "Dal Baati Churma",
      "Mawa Kachori",
      "Boat ride on Lake Pichola",
      "Rooftop dinner overlooking the lake",
      "Puppet show",
      "Bagore Ki Haveli evening show"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "the_hosteller",
      "moustache",
      "madpackers",
      "bloom"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Every rooftop restaurant in the old city has a view that makes you feel like you're in a painting — I didn't want to leave.",
      // "Rent a scooter for a day, drive around Fateh Sagar in the evening, stop for corn on the lakeside — perfection.",
      // "Monsoon Udaipur is secretly magical if you don't mind some rain — the hills turn green and the lakes fill up spectacularly."
    // ]
  },
  "kumbhalgarh": {
    "city": "Kumbhalgarh",
    "lat": 25.1473,
    "lng": 73.5858,
    "state": "rajasthan",
    "region": "northern",
    "category": [
      "forts_palaces"
    ],
    "must_visit": [
      "Kumbhalgarh Fort",
      "The Great Wall of India (fort walls)",
      "Badal Mahal",
      "Kumbhalgarh Wildlife Sanctuary",
      "Neelkanth Mahadev Temple"
    ],
    "must_try": [
      "Dal Baati at a dhaba",
      "Jeep safari in wildlife sanctuary",
      "Sunrise from the fort battlements",
      "Trek along the fort walls"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "Standing on those 36-kilometre walls at sunrise with mist rolling through the Aravalli valleys — this is the image that stays with you long after you leave Rajasthan.",
      // "It's a day-trip from Udaipur for most but staying overnight in the forest means you get the fort completely to yourself at dawn.",
      // "The wildlife sanctuary is underrated — sloth bears and leopards live here and the forest drive is genuinely thrilling."
    // ]
  },
  "kevadiya": {
    "city": "Kevadiya",
    "lat": 21.838,
    "lng": 73.7193,
    "state": "gujarat",
    "region": "western",
    "category": [
      "monuments_museums"
    ],
    "must_visit": [
      "Statue of Unity",
      "Valley of Flowers",
      "Cactus Garden",
      "Butterfly Garden",
      "Zarwani Waterfall",
      "Jungle Safari"
    ],
    "must_try": [
      "Viewing deck at the Statue of Unity",
      "Seaplane experience",
      "Local tribal cuisine",
      "Tent resort stay"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "The statue is genuinely staggering in scale — nothing prepares you for how enormous it is until you're standing at the base looking up.",
      // "Beyond the statue, the surrounding eco-tourism zone is well-designed and surprisingly green — not the dusty construction-site vibe I expected.",
      // "It's a government project so it's all very organised and a bit theme-park-y, but worth a day trip from Vadodara or Surat."
    // ]
  },
  "surat": {
    "city": "Surat",
    "lat": 21.1702,
    "lng": 72.8311,
    "state": "gujarat",
    "region": "western",
    "category": [
      "metro",
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "Surat Castle",
      "Dutch Cemetery",
      "Sardar Patel Museum",
      "Dumas Beach",
      "Swaminarayan Temple",
      "Gopi Talav"
    ],
    "must_try": [
      "Surti Locho",
      "Ghari Mithai",
      "Undhiyu",
      "Ponk (winter season)",
      "Sev Khamani",
      "Diamond industry tour"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Surat is primarily a food city — if you're not eating every two hours you're doing it wrong.",
      // "The textile and diamond markets are fascinating to walk through even if you're not buying — the scale of trade happening in narrow lanes is mind-boggling.",
      // "Not a typical tourist destination but has a certain unpretentious energy that's refreshing after Rajasthan's tourism machinery."
    // ]
  },
  "vadodara": {
    "city": "Vadodara",
    "lat": 22.29941,
    "lng": 73.20812,
    "state": "gujarat",
    "region": "western",
    "category": [
      "forts_palaces",
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "Laxmi Vilas Palace",
      "Sayaji Bagh (Kamati Baug)",
      "Maharaja Fateh Singh Museum",
      "EME Temple",
      "Champaner-Pavagadh"
    ],
    "must_try": [
      "Sev Usal",
      "Dabeli",
      "Surti Undhiyu",
      "Baroda Museum visit",
      "Navratri garba (if in season)"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Laxmi Vilas Palace is four times the size of Buckingham Palace and still a royal residence — that alone makes Vadodara worth a stop.",
      // "The Baroda Museum inside the park is one of those old colonial-era institutions where time has stopped in the best possible way.",
      // "Day trip to Champaner-Pavagadh is essential — a UNESCO site that most tourists completely skip."
    // ]
  },
  "somnath": {
    "city": "Somnath",
    "lat": 20.888,
    "lng": 70.4012,
    "state": "gujarat",
    "region": "western",
    "category": [
      "pilgrimage",
      "coastal"
    ],
    "must_visit": [
      "Somnath Temple",
      "Triveni Sangam",
      "Bhalka Tirth",
      "Dehotsarg Tirth",
      "Somnath Beach"
    ],
    "must_try": [
      "Evening aarti at the temple",
      "Prabhavati Prasad",
      "Seafood at Veraval harbour",
      "Sunset at the arrow pillar (baan stambh)",
      "Morning darshan at 7am"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "The arrow pillar at the ocean edge — pointing into the sea with 'there is no land between here and Antarctica' — gave me chills I didn't expect from a pilgrimage site.",
      // "The evening sound and light show is surprisingly well-produced and puts the temple's history of destruction and rebuilding into stark relief.",
      // "Very crowded on weekends and festival days — arrive on a Tuesday morning for a completely different experience."
    // ]
  },
  "dwarka": {
    "city": "Dwarka",
    "lat": 22.23944,
    "lng": 68.96778,
    "state": "gujarat",
    "region": "western",
    "category": [
      "pilgrimage",
      "coastal"
    ],
    "must_visit": [
      "Dwarkadhish Temple",
      "Bet Dwarka",
      "Nageshwar Jyotirlinga",
      "Rukmini Temple",
      "Lighthouse Beach"
    ],
    "must_try": [
      "Morning aarti at Dwarkadhish",
      "Boat trip to Bet Dwarka",
      "Panchkui well visit",
      "Sudarshan Chakra darshan from outside",
      "Seafood on the harbour"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Dwarka has a mythological weight that very few places carry — standing by the sea knowing this was supposedly Krishna's kingdom is unexpectedly moving even for a non-devotee.",
      // "Bet Dwarka island by boat is the more intimate part of the pilgrimage — the lanes of the island feel genuinely ancient.",
      // "The town itself is small and manageable unlike the chaos of most pilgrimage towns — easier to be present and unhurried here."
    // ]
  },
  "rajkot": {
    "city": "Rajkot",
    "lat": 22.29161,
    "lng": 70.79322,
    "state": "gujarat",
    "region": "western",
    "category": [
      "metro",
      "in_transit"
    ],
    "must_visit": [
      "Watson Museum",
      "Jubilee Garden",
      "Rotary Doll Museum",
      "Kaba Gandhi No Delo",
      "Rajkumar College"
    ],
    "must_try": [
      "Sev Khamani",
      "Chevdo",
      "Bhajiya at roadside stalls",
      "Gujarati thali",
      "Visit to Gondal (day trip)"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Rajkot is Gandhi's childhood city and the museum there does a good job of showing the domestic side of his early life — intimate rather than monumental.",
      // "Saurashtra's business hub isn't glamorous but is a useful base for exploring the Kathiawar peninsula.",
      // "Watson Museum is genuinely one of the better colonial-era district museums in India — worth an hour of your time."
    // ]
  },
  "ludhiana": {
    "city": "Ludhiana",
    "lat": 30.91204,
    "lng": 75.85379,
    "state": "punjab",
    "region": "northern",
    "category": [
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "Punjab Agricultural University Museum",
      "Nehru Rose Garden",
      "Lodhi Fort",
      "Deer Park",
      "Rakh Bagh"
    ],
    "must_try": [
      "Makki di roti and sarson da saag",
      "Lassi at local dairy shops",
      "Amritsari kulcha at highway dhabas",
      "Pinni and gajak in winter",
      "Street chaat near Ghanta Ghar"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Ludhiana is the industrial heartland of Punjab — not a tourist city but the dhabas on the GT Road are exceptional.",
      // "Best used as an overnight stop between Chandigarh and Amritsar rather than a destination.",
      // "The Punjabi hospitality is real here — any local you ask will overfeed you."
    // ]
  },
  "jalandhar": {
    "city": "Jalandhar",
    "lat": 31.32556,
    "lng": 75.57917,
    "state": "punjab",
    "region": "northern",
    "category": [
      "in_transit"
    ],
    "must_visit": [
      "Devi Talab Mandir",
      "Imam Nasir Mosque",
      "Company Bagh",
      "Rangla Punjab Haveli",
      "Sports goods market"
    ],
    "must_try": [
      "Jalandhari lassi",
      "Desi ghee parathas",
      "Amritsari fish tikka",
      "Pinni from halwai shops",
      "Visiting the famous sports goods district"
    ],
    "accommodation": [
      "bloom"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1 day",
    "rating": 2,
    // "notes": [
      // "Jalandhar is primarily a transit point on the Delhi-Amritsar highway — the sports goods market is genuinely worth a browse.",
      // "Not a stand-alone destination but worth a half-day stop if driving through.",
      // "Devi Talab temple has a certain old-world calm that contrasts nicely with the city's commercial vibe."
    // ]
  },
  "amritsar": {
    "city": "Amritsar",
    "lat": 31.62234,
    "lng": 74.87534,
    "state": "punjab",
    "region": "northern",
    "category": [
      "pilgrimage",
      "forts_palaces",
      "monuments_museums",
      "border_town"
    ],
    "must_visit": [
      "Golden Temple (Harmandir Sahib)",
      "Jallianwala Bagh",
      "Partition Museum",
      "Gobindgarh Fort",
      "Wagah Border Ceremony",
      "Durgiana Temple"
    ],
    "must_try": [
      "Langar at Golden Temple",
      "Amritsari kulcha at Kulcha Land",
      "Amritsari fish at Hall Bazar",
      "Lassi at Brothers Dhaba",
      "Wagah Border Beating Retreat ceremony",
      "Evening at Golden Temple for illumination",
      "Makki di roti at a highway dhaba on the way back"
    ],
    "accommodation": [
      "gostops",
      "the_hosteller",
      "madpackers",
      "bloom"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 5,
    // "notes": [
      // "The Golden Temple at 3am after the palki sahib procession is one of the most powerful things you can witness in India.",
      // "Langar feeds thousands every day for free — sitting on the floor and eating with strangers is a lesson in human dignity.",
      // "Wagah Border feels theatrical until the crowd emotion hits you — go for the last ceremony of the day.",
      // "The Partition Museum is essential, not optional — give it two full hours."
    // ]
  },
  "pathankot": {
    "city": "Pathankot",
    "lat": 32.27484,
    "lng": 75.65287,
    "state": "punjab",
    "region": "northern",
    "category": [
      "border_town",
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "Mukteshwar Temple",
      "Shahpurkandi Fort",
      "Ranjit Sagar Dam",
      "Kathgarh Temple",
      "Triund viewpoint (day trip)"
    ],
    "must_try": [
      "Punjabi dhabas on the highway",
      "Fresh fish from Ravi river",
      "Rajma chawal at local joints",
      "Early morning chai at railway station stalls"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1 day",
    "rating": 2,
    // "notes": [
      // "Pathankot is purely a gateway city — you are here to catch a bus or train to Dharamshala, Dalhousie, or Jammu.",
      // "The Ranjit Sagar Dam reservoir is surprisingly scenic for a detour on your way through.",
      // "Stock up on supplies here before heading into the hills — last point of easy availability."
    // ]
  },
  "katra": {
    "city": "Katra",
    "lat": 32.99167,
    "lng": 74.93195,
    "state": "jammu_kashmir",
    "region": "northern",
    "category": [
      "pilgrimage"
    ],
    "must_visit": [
      "Vaishno Devi Temple (trek)",
      "Ardhkuwari Cave",
      "Bhairon Mandir",
      "Banganga",
      "Shiv Khori (day trip)"
    ],
    "must_try": [
      "Vaishno Devi darshan and prasad",
      "Rajma and rice at dhaba near helipad",
      "Hot kahwa on the trek route",
      "Charan Paduka visit at dawn",
      "Pony or palanquin experience on the hill trail"
    ],
    "accommodation": [
      "gostops",
      "bloom"
    ],
    "mode_of_travel": [
      "ola/uber",
      "auto"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "The trek to Vaishno Devi is genuinely uplifting — the energy of tens of thousands walking together in the dark is unlike anything else.",
      // "Book your registration slip well in advance, especially for weekends and Navratri.",
      // "Katra town itself is very small — one full day is enough outside the pilgrimage itself."
    // ]
  },
  "jammu": {
    "city": "Jammu",
    "lat": 32.73569,
    "lng": 74.86911,
    "state": "jammu_kashmir",
    "region": "northern",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "pilgrimage",
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "Raghunath Temple",
      "Bahu Fort & Garden",
      "Amar Mahal Palace Museum",
      "Peer Kho Cave Temple",
      "Mubarak Mandi Palace"
    ],
    "must_try": [
      "Kaladi kulcha",
      "Dogri mutton curry",
      "Ambal (Dogri dish)",
      "Patisa and halwa from sweet shops",
      "Evening walk at Raghunath Bazaar",
      "Gondola ride over Tawi river"
    ],
    "accommodation": [
      "bloom"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Jammu is the forgotten half of J&K — the Dogri culture and food here are genuinely distinct from both Kashmir and Punjab.",
      // "Kaladi kulcha for breakfast is essential — it is the most unique thing you will eat on the whole J&K stretch.",
      // "Most people rush through Jammu towards Kashmir — slow down and spend a proper day here."
    // ]
  },
  "banihar-qazigund": {
    "city": "Qazigund",
    "lat": 33.5167,
    "lng": 75.2005,
    "state": "jammu_kashmir",
    "region": "northern",
    "category": [],
    "must_visit": [
      "Banihal Tunnel (Jawahar Tunnel) entrance",
      "Banihal Pass viewpoint",
      "Qazigund railway station",
      "Himalayan landscapes along NH44",
      "Ramban gorge views"
    ],
    "must_try": [
      "Nun chai (pink salt tea) at roadside stalls",
      "Roti with local pickle from highway dhabas",
      "Watching the Pir Panjal range from the highway",
      "First snow experience at Banihal in early winter"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [],
    "ideal_season": "post_monsoon",
    "ideal_stay": "1 day",
    "rating": 2,
    // "notes": [
      // "This is a transit stretch, not a destination — but the drive through Ramban gorge is genuinely dramatic.",
      // "The Banihal Tunnel is a lifeline; check tunnel timings and traffic updates before you start.",
      // "Qazigund side feels distinctly Kashmiri — the air, the vegetation, the architecture all change suddenly."
    // ]
  },
  "srinagar": {
    "city": "Srinagar",
    "lat": 34.08565,
    "lng": 74.80555,
    "state": "jammu_kashmir",
    "region": "northern",
    "category": [
      "monuments_museums",
      "hill_station",
      "backpacker"
    ],
    "must_visit": [
      "Dal Lake and Shikara ride",
      "Mughal Gardens (Shalimar, Nishat, Chashme Shahi)",
      "Shankaracharya Temple",
      "Hazratbal Shrine",
      "Lal Chowk"
    ],
    "must_try": [
      "Wazwan meal at a local home or traditional restaurant",
      "Noon chai and girda bread for breakfast",
      "Shikara ride at sunrise on Dal Lake",
      "Rogan josh and gushtaba",
      "Kashmiri kahwa",
      "Strolling the old city bazaar for saffron and dry fruits",
      "Houseboat stay on Dal Lake"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "moustache",
      "bloom"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "bike_rental"
    ],
    "ideal_season": "summer",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "The Mughal Gardens are most beautiful in spring tulip season — late March to mid April is peak.",
      // "Staying in a houseboat even one night is worth the extra money — waking up to mist on Dal Lake is otherworldly.",
      // "The old city bazaar around Lal Chowk has the best saffron and dry fruits, negotiate hard."
    // ]
  },
  "gulmarg": {
    "city": "Gulmarg",
    "lat": 34.0485,
    "lng": 74.3806,
    "state": "jammu_kashmir",
    "region": "northern",
    "category": [
      "hill_station"
    ],
    "must_visit": [
      "Gondola Phase 1 & 2 (Apharwat Peak)",
      "Gulmarg Golf Course",
      "St. Mary's Church",
      "Alpather Lake",
      "Kongdori meadows"
    ],
    "must_try": [
      "Gondola ride to Apharwat Peak",
      "Skiing or snowboarding in winter",
      "Hot Kashmiri kahwa in the snow",
      "Sledding on the slopes",
      "Walking through the pine forest",
      "Noon chai with bakarkhani"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 5,
    // "notes": [
      // "In deep winter the gondola ride to Apharwat is genuinely world-class skiing terrain at a fraction of European prices.",
      // "Summer Gulmarg is underrated — vast green meadows with the snowline visible above are stunning.",
      // "Come for at least two nights — one day is not enough to absorb the altitude and atmosphere."
    // ]
  },
  "kargil": {
    "city": "Kargil",
    "lat": 34.55765,
    "lng": 76.12622,
    "state": "ladakh",
    "region": "northern",
    "category": [
      "border_town",
      "monuments_museums"
    ],
    "must_visit": [
      "Kargil War Memorial (Dras)",
      "Suru Valley",
      "Mulbekh Monastery",
      "Rangdum Monastery",
      "Drass Village"
    ],
    "must_try": [
      "Butter tea (gurgur chai)",
      "Mutton thukpa at local dhabas",
      "Visiting Dras — coldest inhabited place in India",
      "Standing at LOC viewpoint",
      "Apricot picking in season (July-August)",
      "Stargazing at high altitude"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [],
    "ideal_season": "summer",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "Kargil War Memorial at Dras is deeply affecting — read the names on the wall and give it the silence it deserves.",
      // "Most people rush through Kargil on the Srinagar-Leh highway, which is a mistake — Suru Valley is spectacular.",
      // "The local apricots in July are some of the best you will ever eat — buy them from roadside sellers."
    // ]
  },
  "leh": {
    "city": "Leh",
    "lat": 34.16504,
    "lng": 77.58402,
    "state": "ladakh",
    "region": "northern",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "pilgrimage",
      "backpacker"
    ],
    "must_visit": [
      "Leh Palace",
      "Thiksey Monastery",
      "Pangong Tso Lake",
      "Nubra Valley & Khardung La",
      "Hemis Monastery"
    ],
    "must_try": [
      "Thukpa and thenthuk (Ladakhi noodle soups)",
      "Butter tea at a monastery",
      "Skyu (traditional pasta stew)",
      "Motorcycle ride on Khardung La or Leh-Manali highway",
      "Sunrise at Pangong Tso",
      "Visit a local homestay in Nubra",
      "Camping under stars at Pangong"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "the_hosteller"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "summer",
    "ideal_stay": "1 week",
    "rating": 5,
    // "notes": [
      // "Acclimatize for at least two full days in Leh before any high-altitude excursion — altitude sickness is real and serious.",
      // "Pangong Tso at first light with no other cars around is one of the most surreal experiences available in India.",
      // "Leh is expensive relative to the rest of India — budget accordingly, especially for taxis and permits."
    // ]
  },
  "keylong": {
    "city": "Keylong",
    "lat": 32.5681,
    "lng": 77.0241,
    "state": "himachal_pradesh",
    "region": "northern",
    "category": [],
    "must_visit": [
      "Keylong Monastery (Kardang Gompa)",
      "Triloknath Temple",
      "Shashur Monastery",
      "Bhaga River banks",
      "Baralacha La viewpoint"
    ],
    "must_try": [
      "Butter tea at a local guesthouse",
      "Tsampa (roasted barley meal)",
      "Trout from Bhaga river",
      "Walking to Kardang Gompa",
      "Night sky stargazing",
      "High mountain pass drive to Baralacha La"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "summer",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Keylong is a pit stop between Manali and Leh — but the Kardang Gompa is genuinely worth an hour's walk.",
      // "The town sits in a barren valley that looks almost Tibetan — the desolation is beautiful if you sit with it.",
      // "Temperatures drop sharply at night even in July — always carry warm layers."
    // ]
  },
  "manali": {
    "city": "Manali",
    "lat": 32.2396,
    "lng": 77.1887,
    "state": "himachal_pradesh",
    "region": "northern",
    "category": [
      "hill_station",
      "backpacker"
    ],
    "must_visit": [
      "Rohtang Pass",
      "Hadimba Devi Temple",
      "Solang Valley",
      "Old Manali",
      "Beas Kund Trek"
    ],
    "must_try": [
      "Siddu with ghee and dal",
      "Trout at riverside cafes in Old Manali",
      "Paragliding at Solang Valley",
      "Snow activities at Rohtang",
      "Apple cider from Kullu valley orchards",
      "Sunrise trek to Beas Kund base",
      "Evening bonfire in Old Manali"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "the_hosteller",
      "moustache",
      "madpackers"
    ],
    "mode_of_travel": [
      "bike_rental",
      "ola/uber"
    ],
    "ideal_season": "summer",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Old Manali has become a bit backpacker-touristy but it still has the best cafes and river-facing guesthouses.",
      // "Rohtang is worth the early 4am start — once crowds arrive after 9am, the magic evaporates.",
      // "Manali in October post-season is quieter, cheaper, and the autumn colours are extraordinary."
    // ]
  },
  "kullu": {
    "city": "Kullu",
    "lat": 31.95835,
    "lng": 77.10823,
    "state": "himachal_pradesh",
    "region": "northern",
    "category": [
      "hill_station"
    ],
    "must_visit": [
      "Raghunath Temple",
      "Great Himalayan National Park",
      "Bijli Mahadev Temple",
      "Kullu Dussehra ground",
      "Kasol day trip (Parvati Valley)"
    ],
    "must_try": [
      "Dham (traditional Kullu feast)",
      "River rafting on Beas",
      "Apple buying directly from orchards in season",
      "Trout fishing at Tirthan Valley",
      "Local wool shawl shopping at Akhara Bazaar"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "city_bus",
      "bike_rental"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Kullu Dussehra is one of the most unique festivals in India — hundreds of local devtas (deities) are carried in procession.",
      // "Most travellers skip Kullu and go straight to Manali, which means you get the valley views without the crowd.",
      // "The Tirthan Valley nearby is genuinely off the radar — exactly what Manali used to be 15 years ago."
    // ]
  },
  "chandigarh": {
    "city": "Chandigarh",
    "lat": 30.73629,
    "lng": 76.7884,
    "state": "chandigarh",
    "region": "northern",
    "category": [
      "monuments_museums",
      "base_camp"
    ],
    "must_visit": [
      "Rock Garden (Nek Chand)",
      "Sukhna Lake",
      "Rose Garden (Zakir Hussain Garden)",
      "Capitol Complex (Le Corbusier)",
      "Government Museum & Art Gallery"
    ],
    "must_try": [
      "Butter chicken at any Sector 22 restaurant",
      "Chole kulche at Sector 22 chowk",
      "Lassi at Gopal's",
      "Evening cycling around Sukhna Lake",
      "Walk through Rock Garden — give it two full hours"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "ola/uber",
      "auto",
      "city_bus",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "The Rock Garden is one of the most original outsider-art environments in the world — completely unexpected in a planned government city.",
      // "Chandigarh is the cleanest and most legible city in north India — it's genuinely pleasant just to walk around.",
      // "The Capitol Complex is a UNESCO site but often overlooked — Le Corbusier nerds will lose their minds here."
    // ]
  },
  "bikaner": {
    "city": "Bikaner",
    "lat": 28.01762,
    "lng": 73.31495,
    "state": "rajasthan",
    "region": "northern",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "desert"
    ],
    "must_visit": [
      "Junagarh Fort",
      "Karni Mata Temple (Deshnoke)",
      "Lalgarh Palace",
      "Bhandasar Jain Temple",
      "Ganga Government Museum"
    ],
    "must_try": [
      "Bikaneri Bhujia",
      "Rasgulla (local version)",
      "Camel safari",
      "Camel breeding farm visit",
      "Kote Gate bazaar walk"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "Junagarh Fort is the real sleeper hit of Rajasthan — almost no tourists, insanely intricate interiors, and zero commercialisation.",
      // "The Rat Temple at Deshnoke is either the most disturbing or most fascinating thing you'll see in India — go early when the rats are most active.",
      // "Bikaneri bhujia from the original Haldiram's in town tastes completely different from anything you get in a packet — transformative."
    // ]
  },
  "sam": {
    "city": "Sam",
    "lat": 26.889,
    "lng": 70.7158,
    "state": "rajasthan",
    "region": "northern",
    "category": [
      "desert"
    ],
    "must_visit": [
      "Sam Sand Dunes",
      "Sunset point at the dunes",
      "Desert Camp",
      "Stargazing spot",
      "Camel safaris"
    ],
    "must_try": [
      "Camel ride at sunset",
      "Overnight desert camp stay",
      "Folk music and dance performance at camp",
      "Sleeping under open sky",
      "Rajasthani dal baati at camp"
    ],
    "accommodation": [
      "zostel"
    ],
    "mode_of_travel": [],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "The silence of the Sam dunes at 5am, before the camel touts arrive, is one of those rare moments that makes you understand why people travel.",
      // "Don't skip the overnight camp — watching the Milky Way from a Thar Desert dune is not something you can simulate anywhere else in India.",
      // "Avoid the peak tourist months of December-January if you want any solitude — the dunes get alarmingly crowded otherwise."
    // ]
  },
  "jaisalmer": {
    "city": "Jaisalmer",
    "lat": 26.91763,
    "lng": 70.90387,
    "state": "rajasthan",
    "region": "northern",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "desert"
    ],
    "must_visit": [
      "Jaisalmer Fort (Sonar Qila)",
      "Patwon Ki Haveli",
      "Salim Singh Ki Haveli",
      "Gadisar Lake",
      "Sam Sand Dunes"
    ],
    "must_try": [
      "Ker Sangri",
      "Mirchi Bada",
      "Rooftop chai overlooking the fort",
      "Desert jeep safari",
      "Camel trek to village",
      "Sunset at the dunes"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller",
      "moustache"
    ],
    "mode_of_travel": [
      "auto",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 5,
    // "notes": [
      // "Jaisalmer fort is a living fort — people still live, cook and run shops inside — and that makes it feel like a time machine rather than a museum.",
      // "The havelis here are the finest examples of sandstone carving you'll ever see, and most of the tourists walk right past them to get to the dunes.",
      // "Rent a bike and drive to the desert villages beyond Sam — that's where the real Rajasthan lives, not in the camel selfie zone."
    // ]
  },
  "jodhpur": {
    "city": "Jodhpur",
    "lat": 26.26841,
    "lng": 73.00594,
    "state": "rajasthan",
    "region": "northern",
    "category": [
      "forts_palaces",
      "monuments_museums"
    ],
    "must_visit": [
      "Mehrangarh Fort",
      "Jaswant Thada",
      "Umaid Bhawan Palace",
      "Ghanta Ghar (Clock Tower)",
      "Mandore Gardens"
    ],
    "must_try": [
      "Makhaniya Lassi",
      "Mirchi Vada",
      "Mawa Kachori",
      "Blue city walk in old town",
      "Mehrangarh sound and light show",
      "Breakfast at Shri Mishrilal Hotel"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller",
      "moustache",
      "madpackers"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 5,
    // "notes": [
      // "Mehrangarh is the most imposing fort in India — period. Standing at the top looking over the blue city is one of those views that rewires your brain.",
      // "The clock tower market at 8am, buying spices and sipping chai while the city wakes up — that's the real Jodhpur that Instagram misses.",
      // "The blue city is less uniformly blue than photos suggest but wandering those narrow lanes in the morning light is still completely magical."
    // ]
  },
  "ajmer-pushkar": {
    "city": "Ajmer & Pushkar",
    "lat": 26.4521,
    "lng": 74.63867,
    "state": "rajasthan",
    "region": "northern",
    "category": [
      "pilgrimage"
    ],
    "must_visit": [
      "Dargah Sharif of Moinuddin Chishti",
      "Pushkar Lake",
      "Brahma Temple",
      "Ana Sagar Lake",
      "Taragarh Fort",
      "Savitri Mata Temple"
    ],
    "must_try": [
      "Pushkar malpua",
      "Lassi at a lakeside stall",
      "Sunset aarti at Pushkar Ghat",
      "Sufi qawwali at the Dargah",
      "Camel Fair (if in season)",
      "Shopping on Pushkar's main bazaar"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller",
      "moustache",
      "madpackers"
    ],
    "mode_of_travel": [
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Ajmer's Dargah is one of the most powerful religious experiences in India regardless of your faith — the qawwali on Thursday nights is something else entirely.",
      // "Pushkar is Rajasthan's hippie town and the contrast with Ajmer's Islamic spiritual gravity just 11 km away is one of India's great juxtapositions.",
      // "The camel fair in November is absolute madness and absolutely worth planning your entire Rajasthan trip around."
    // ]
  },
  "ahmedabad-gandhinagar": {
    "city": "Ahmedabad & Gandhinagar",
    "lat": 23.02579,
    "lng": 72.58727,
    "state": "gujarat",
    "region": "western",
    "category": [
      "metro",
      "monuments_museums"
    ],
    "must_visit": [
      "Sabarmati Ashram",
      "Adalaj Stepwell",
      "GIFT City",
      "Akshardham Temple Gandhinagar",
      "Sidi Saiyyed Mosque",
      "Science City Ahmedabad",
      "Calico Museum of Textiles",
      "Kankaria Lake",
      "Hutheesing Jain Temple"
    ],
    "must_try": [
      "Gujarati thali",
      "Fafda-Jalebi breakfast",
      "Khaman and Khandvi",
      "Undhiyu",
      "Heritage walk in pol area",
      "Kite festival experience (January)",
      "Manek Chowk night food market",
      "Sabarmati Riverfront evening walk"
    ],
    "accommodation": [
      "bloom"
    ],
    "mode_of_travel": [
      "metro",
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 4,
    // "notes": [
      // "The UNESCO heritage walk through Ahmedabad's old city pols is genuinely one of the finest urban heritage experiences in all of India.",
      // "Gandhinagar is that rare Indian state capital that actually feels planned — clean, green, and oddly calming after Ahmedabad's intensity.",
      // "Manek Chowk transforms from a jewellery market by day to a street food paradise by night — don't make dinner plans, just go there.",
      // "Traffic is a nightmare but the metro has made getting around surprisingly easy now."
    // ]
  },
  "indore": {
    "city": "Indore",
    "lat": 22.71792,
    "lng": 75.8333,
    "state": "madhya_pradesh",
    "region": "central",
    "category": [
      "metro",
      "in_transit"
    ],
    "must_visit": [
      "Rajwada Palace",
      "Lal Bagh Palace",
      "Kanch Mandir",
      "Chappan Dukan (56 Shops)",
      "Central Museum",
      "Patalpani Waterfall"
    ],
    "must_try": [
      "Indori Poha with sev and jalebi",
      "Garadu chaat",
      "Sabudana khichdi",
      "Bhutte ka kees",
      "Sarafa Bazaar night food walk",
      "Chappan Dukan street food crawl"
    ],
    "accommodation": [
      "zostel"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Indore's Sarafa Bazaar at midnight — a jewellery market by day that becomes India's best night food market — is the most unique dining experience I've had in Central India.",
      // "The city is consistently ranked India's cleanest for a reason — Indori pride in their city is tangible and infectious.",
      // "Come hungry and plan no fixed itinerary — the best Indore experiences happen when you follow your nose through the lanes."
    // ]
  },
  "omkareshwar": {
    "city": "Omkareshwar",
    "lat": 22.2334,
    "lng": 76.1512,
    "state": "madhya_pradesh",
    "region": "central",
    "category": [
      "pilgrimage"
    ],
    "must_visit": [
      "Omkareshwar Jyotirlinga Temple",
      "Mamleshwar Temple",
      "Island circumambulation (parikrama)",
      "Satmatrika Temple",
      "Narmada River ghats"
    ],
    "must_try": [
      "Morning aarti at the temple",
      "Boat ride on Narmada",
      "Narmada parikrama on foot",
      "Prasad thali",
      "Sunset meditation at the ghat"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "The island itself is shaped like the OM symbol — something about knowing that as you walk the parikrama path above the Narmada gorge makes the whole experience quite otherworldly.",
      // "Far less crowded than Ujjain but just as spiritually charged — this is where you come if you want Narmada darshan without the tourist machinery.",
      // "The boat ride on the Narmada between the two temples at dawn, when the mist is still on the water, is genuinely serene."
    // ]
  },
  "nashik": {
    "city": "Nashik",
    "lat": 19.99727,
    "lng": 73.79096,
    "state": "maharashtra",
    "region": "western",
    "category": [
      "pilgrimage"
    ],
    "must_visit": [
      "Trimbakeshwar Temple",
      "Panchvati (Sita Gufa, Kalaram Temple)",
      "Pandavleni Caves",
      "Nashik wineries (Sula, York)",
      "Ramkund"
    ],
    "must_try": [
      "Misal Pav",
      "Sabudana vada",
      "Wine tasting at Sula Vineyards",
      "Kumbh Mela darshan (if in cycle)",
      "Grape-stomping experience (February)",
      "Trimbakeshwar aarti"
    ],
    "accommodation": [
      "zostel"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "The bizarre combination of being a major Hindu pilgrimage town AND India's wine capital produces a uniquely schizophrenic energy — I love it.",
      // "Sula Vineyards at harvest time in February is legitimately world-class and totally unexpected in India's interior.",
      // "Trimbakeshwar has some of the most ornate temple architecture in Maharashtra and is always less mobbed than it deserves to be."
    // ]
  },
  "shirdi": {
    "city": "Shirdi",
    "lat": 19.76616,
    "lng": 74.47738,
    "state": "maharashtra",
    "region": "western",
    "category": [
      "pilgrimage"
    ],
    "must_visit": [
      "Sai Baba Samadhi Mandir",
      "Dwarkamai Masjid",
      "Chavadi",
      "Gurusthan",
      "Shani Shingnapur (day trip)"
    ],
    "must_try": [
      "Temple prasad",
      "Breakfast at the prasadalaya",
      "Early morning darshan queue",
      "Palki procession",
      "Shej aarti"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto"
    ],
    "ideal_season": "year_round",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "The devotional energy here is completely overwhelming in a way that's hard to prepare for — tens of thousands of people per day and yet the atmosphere is strangely calm.",
      // "Spend the night and catch the early morning Kakad aarti at 4:30am — the experience when the temple is half-lit and the bhajans echo off the stone is transformative.",
      // "The commercial zone around the temple is aggressively tacky — keep your eyes forward and walk straight to the complex."
    // ]
  },
  "sambhajinagar": {
    "city": "Sambhajinagar",
    "lat": 19.8762,
    "lng": 75.3433,
    "state": "maharashtra",
    "region": "western",
    "category": [
      "forts_palaces",
      "monuments_museums"
    ],
    "must_visit": [
      "Ellora Caves",
      "Ajanta Caves",
      "Bibi Ka Maqbara",
      "Daulatabad Fort",
      "Panchakki"
    ],
    "must_try": [
      "Naan Qalia (Mughal-era mutton dish)",
      "Biryani at old city restaurants",
      "Full day at Ellora",
      "Sunrise at Ajanta",
      "Daulatabad fort climb"
    ],
    "accommodation": [
      "zostel"
    ],
    "mode_of_travel": [
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Ellora's Kailasa Temple — carved downward from a single rock — is architecturally the most mind-bending structure I have ever stood inside.",
      // "Ajanta needs a dawn arrival; the painted caves in early light before the tourist buses arrive are exactly why people call this a world wonder.",
      // "Budget three full days minimum — Ellora, Ajanta, and Daulatabad each deserve a dedicated day and most tourists catastrophically underestimate this."
    // ]
  },
  "pune": {
    "city": "Pune",
    "lat": 18.51957,
    "lng": 73.85535,
    "state": "maharashtra",
    "region": "western",
    "category": [
      "metro",
      "forts_palaces",
      "monuments_museums",
      "base_camp"
    ],
    "must_visit": [
      "Shaniwar Wada",
      "Aga Khan Palace",
      "Sinhagad Fort",
      "Osho International Meditation Resort",
      "Pataleshwar Cave Temple",
      "Dagdusheth Halwai Ganapati"
    ],
    "must_try": [
      "Misal Pav",
      "Vada Pav",
      "Pune Special Thali",
      "Koregaon Park café hopping",
      "Trekking to Sinhagad",
      "Pune FC football match (if in season)"
    ],
    "accommodation": [
      "zostel",
      "bloom"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 4,
    // "notes": [
      // "Pune is one of the most liveable cities in India — the Sahyadri backdrop, the student energy, the café culture and the genuinely good weather makes you want to just stay.",
      // "Sinhagad at 5am before the weekend crowds hit — misty, silent, with tea and bhurji at the top — is one of Maharashtra's perfect mornings.",
      // "The city has gentrified rapidly but the old Peth areas still feel like 18th-century Maratha pulse — walk through Kasba and Mandai market."
    // ]
  },
  "mumbai": {
    "city": "Mumbai",
    "lat": 19.07283,
    "lng": 72.88261,
    "state": "maharashtra",
    "region": "western",
    "category": [
      "metro",
      "coastal",
      "base_camp"
    ],
    "must_visit": [
      "Gateway of India",
      "Marine Drive",
      "Elephanta Caves",
      "Dharavi",
      "Chhatrapati Shivaji Maharaj Terminus (CST)",
      "Bandra-Worli Sea Link"
    ],
    "must_try": [
      "Vada Pav at Kirti College",
      "Pav Bhaji at Juhu Beach",
      "Seafood at Gajalee or Trishna",
      "Local train experience",
      "Dharavi slum walk",
      "Sanjay Gandhi NP leopard safari",
      "Chowpatty beach bhel"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller",
      "bloom"
    ],
    "mode_of_travel": [
      "metro",
      "city_bus",
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Mumbai is the only Indian city where the local train at rush hour feels like a rite of passage rather than an ordeal — do it at least once, it will teach you something.",
      // "Marine Drive at 6am, watching the city wake up over the Arabian Sea with a cutting chai from a nearby tapri — this is the real Mumbai.",
      // "The city never stops, never sleeps, never apologises for being itself — exhausting and completely intoxicating in equal measure."
    // ]
  },
  "goa": {
    "city": "Goa",
    "lat": 15.49574,
    "lng": 73.82624,
    "state": "goa",
    "region": "western",
    "category": [
      "coastal",
      "backpacker",
      "forts_palaces",
      "monuments_museums"
    ],
    "must_visit": [
      "Calangute and Baga Beach",
      "Palolem Beach",
      "Basilica of Bom Jesus",
      "Se Cathedral",
      "Dudhsagar Falls",
      "Fort Aguada"
    ],
    "must_try": [
      "Fish curry rice",
      "Xacuti",
      "Bebinca",
      "Feni cocktail",
      "Cashew feni tasting",
      "Beach shack breakfast",
      "Spice plantation tour",
      "Saturday Night Market"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "the_hosteller",
      "moustache",
      "madpackers",
      "bloom"
    ],
    "mode_of_travel": [
      "bike_rental",
      "ola/uber",
      "private_taxi"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1 week",
    "rating": 5,
    // "notes": [
      // "South Goa is a completely different universe from North Goa — if your image of Goa is Baga-beach chaos, drive south and recalibrate entirely.",
      // "Rent a bike, ignore the beach shack raves, and just ride through the inland villages of Ponda and Divar island — that's where the real Goa magic lives.",
      // "Dudhsagar in monsoon is worth the muddy jeep trek — the waterfall in full force is one of those absurdly beautiful things."
    // ]
  },
  "kochi": {
    "city": "Kochi",
    "lat": 9.93988,
    "lng": 76.26022,
    "state": "kerala",
    "region": "southern",
    "category": [
      "metro",
      "forts_palaces",
      "monuments_museums",
      "coastal",
      "backpacker",
      "base_camp"
    ],
    "must_visit": [
      "Fort Kochi",
      "Mattancherry Palace",
      "Jewish Synagogue",
      "Chinese Fishing Nets",
      "Marine Drive",
      "Cherai Beach"
    ],
    "must_try": [
      "Kerala fish curry with red rice",
      "prawn moilee",
      "duck roast",
      "appam with stew",
      "backwater ferry ride",
      "kathakali performance at Kerala Kathakali Centre",
      "Jew Town antique shopping",
      "sunset walk on Marine Drive"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "the_hosteller",
      "bloom"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 5,
    // "notes": [
      // "Fort Kochi at dusk felt like stepping into a Portuguese colonial painting — the spice smell and sea breeze hit differently here.",
      // "The ferry from Ernakulam to Fort Kochi is Rs 5 and gives you better views than any tourist boat.",
      // "Mattancherry's Jew Town is chaotic and crowded but the antique shops are genuinely fascinating — stumbled upon 200-year-old maps."
    // ]
  },
  "allepey": {
    "city": "Allepey",
    "lat": 9.49004,
    "lng": 76.3264,
    "state": "kerala",
    "region": "southern",
    "category": [
      "backpacker",
      "coastal"
    ],
    "must_visit": [
      "Alleppey Backwaters",
      "Vembanad Lake",
      "Alappuzha Beach",
      "Kuttanad (Venice of the East)",
      "Marari Beach",
      "Pathiramanal Bird Sanctuary"
    ],
    "must_try": [
      "overnight houseboat stay",
      "toddy with karimeen (pearl spot fish)",
      "crab curry at a local shack",
      "village canoe ride at dawn",
      "sunrise on the backwaters",
      "coconut prawn curry",
      "kayaking through narrow canals"
    ],
    "accommodation": [
      "zostel",
      "gostops"
    ],
    "mode_of_travel": [
      "auto",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 5,
    // "notes": [
      // "The houseboat is overpriced if you book through hotels — find local operators at the jetty and negotiate directly for half the cost.",
      // "Woke up at 5am on the houseboat to complete silence and mist on the water — worth every rupee.",
      // "Kuttanad is rice farming below sea level; watching paddy fields surrounded by canals is genuinely surreal geography."
    // ]
  },
  "varkala": {
    "city": "Varkala",
    "lat": 8.7333,
    "lng": 76.7167,
    "state": "kerala",
    "region": "southern",
    "category": [
      "coastal",
      "backpacker",
      "pilgrimage"
    ],
    "must_visit": [
      "Varkala Cliff",
      "Papanasam Beach",
      "Janardana Swami Temple",
      "Black Beach",
      "Kappil Lake",
      "Sivagiri Mutt"
    ],
    "must_try": [
      "cliff-top café breakfast with sea view",
      "fresh seafood grilled to order on the cliff",
      "cliff paragliding",
      "sunset yoga session",
      "ayurvedic massage",
      "tender coconut on the beach",
      "Janardhana Swami Temple darshan at sunset"
    ],
    "accommodation": [
      "zostel"
    ],
    "mode_of_travel": [
      "auto",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Varkala cliff is Goa for people who've outgrown Goa — same vibe but quieter, cleaner, and with an actual temple.",
      // "The cliff path gets touristy fast but walk 10 minutes north of the main stretch and it's suddenly peaceful.",
      // "Avoid monsoon — the cliffs erode dangerously and most shacks shut down."
    // ]
  },
  "trivandrum": {
    "city": "Trivandrum",
    "lat": 8.4855,
    "lng": 76.94924,
    "state": "kerala",
    "region": "southern",
    "category": [
      "metro",
      "pilgrimage",
      "coastal"
    ],
    "must_visit": [
      "Padmanabhaswamy Temple",
      "Kovalam Beach",
      "Napier Museum",
      "Kuthiramalika Palace",
      "Ponmudi Hill Station",
      "Attukal Bhagavathy Temple"
    ],
    "must_try": [
      "puttu and kadala curry breakfast",
      "Kerala sadhya on banana leaf",
      "Padmanabhaswamy Temple darshan (queue early)",
      "Kovalam lighthouse climb",
      "Neyatinkara border market shopping",
      "fresh coconut water at Kovalam"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "city_bus",
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Padmanabhaswamy Temple has strict dress codes but the experience inside is genuinely spiritual — one of the richest temples in the world.",
      // "Kovalam is touristy but Lighthouse Beach at dawn before the crowds is lovely.",
      // "Trivandrum has surprisingly good café culture and a relaxed pace for a state capital."
    // ]
  },
  "vagamon": {
    "city": "Vagamon",
    "lat": 9.58692,
    "lng": 76.52132,
    "state": "kerala",
    "region": "southern",
    "category": [
      "hill_station",
      "backpacker"
    ],
    "must_visit": [
      "Vagamon Meadows",
      "Thangal Para",
      "Kurisumala Ashram",
      "Murugan Para",
      "Pine Forest",
      "Vagamon Lake"
    ],
    "must_try": [
      "paragliding over the meadows",
      "trekking to Thangal Para at sunrise",
      "fresh strawberries from local farms",
      "campfire night stay",
      "misty morning walk through pine forest",
      "local village chai"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Vagamon is what Munnar was 15 years ago — untouched, misty, and relatively unknown. Don't wait too long.",
      // "The paragliding here is genuinely thrilling — wide open meadows give you full sky views with no obstructions.",
      // "Roads are narrow and steep; a scooter rental makes exploring the surrounding hills much more fun."
    // ]
  },
  "coimbatore": {
    "city": "Coimbatore",
    "lat": 11.00555,
    "lng": 76.96612,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "metro",
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "Isha Yoga Centre & Dhyanalinga",
      "Marudhamalai Temple",
      "VOC Park & Zoo",
      "Perur Pateeswarar Temple",
      "Black Thunder Water Park",
      "Siruvani Waterfalls"
    ],
    "must_try": [
      "Coimbatore idli-sambar breakfast at Annapoorna",
      "parotta with salna",
      "visit Dhyanalinga meditation dome",
      "evening at Brookefields mall",
      "Salem-style biryani",
      "filter coffee at any old-school mess",
      "Coimbatore banana chips"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "city_bus",
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Coimbatore is less a tourist city and more a gateway — to Ooty, Munnar, Wayanad, and Isha.",
      // "The Isha Yoga Centre alone justifies a stop here — the Dhyanalinga is one of the most energetically powerful spaces I've been in.",
      // "Great food city but not much to see beyond temples and the Isha campus."
    // ]
  },
  "ooty": {
    "city": "Ooty",
    "lat": 11.4102,
    "lng": 76.695,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "hill_station"
    ],
    "must_visit": [
      "Nilgiri Mountain Railway (Toy Train)",
      "Ooty Lake",
      "Botanical Gardens",
      "Doddabetta Peak",
      "Rose Garden",
      "Pykara Lake & Waterfalls"
    ],
    "must_try": [
      "toy train ride from Mettupalayam to Ooty",
      "Ooty homemade chocolate from local shops",
      "fresh eucalyptus oil buying",
      "horse riding at Ooty Lake",
      "lamb stew with bread at a local bakery",
      "varkey biscuits",
      "Nilgiri tea tasting"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "the_hosteller"
    ],
    "mode_of_travel": [
      "auto",
      "city_bus",
      "private_taxi"
    ],
    "ideal_season": "summer",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "The toy train is slow and rickety and absolutely magical — book months ahead for weekends.",
      // "Ooty gets brutally crowded in May-June school holidays; visit in March or September for the best experience.",
      // "The Botanical Gardens in full bloom during spring is genuinely worth the entry fee unlike most such places."
    // ]
  },
  "mysore": {
    "city": "Mysore",
    "lat": 12.29791,
    "lng": 76.63925,
    "state": "karnataka",
    "region": "southern",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "pilgrimage"
    ],
    "must_visit": [
      "Mysore Palace",
      "Chamundeshwari Temple",
      "Brindavan Gardens",
      "St. Philomena's Cathedral",
      "Mysore Zoo",
      "Devaraja Market"
    ],
    "must_try": [
      "Mysore pak from Guru Sweet Mart",
      "Mysore masala dosa",
      "palace illumination on Sunday evenings",
      "royal dasara procession (Oct)",
      "Devaraja Market flower and spice shopping",
      "yoga class (Mysore is the ashtanga yoga capital)",
      "Mysore sandalwood soap buying"
    ],
    "accommodation": [
      "zostel"
    ],
    "mode_of_travel": [
      "auto",
      "city_bus",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "The palace on a Sunday evening with 100,000 bulbs lit up is one of the most dramatic sights in India — completely free.",
      // "Mysore is compact and walkable for a city; hired a cycle for a day and covered most of the old city.",
      // "Devaraja Market in the morning smells incredible — jasmine garlands stacked taller than people."
    // ]
  },
  "wayanad": {
    "city": "Wayanad",
    "lat": 11.6854,
    "lng": 76.132,
    "state": "kerala",
    "region": "southern",
    "category": [
      "hill_station",
      "monuments_museums"
    ],
    "must_visit": [
      "Edakkal Caves",
      "Chembra Peak",
      "Banasura Sagar Dam",
      "Pookode Lake",
      "Soochipara Waterfalls",
      "Thirunelli Temple"
    ],
    "must_try": [
      "trek to Chembra Peak (heart-shaped lake at top)",
      "bamboo rafting at Pookode Lake",
      "Wayanad tribal village visit",
      "bamboo rice puttu",
      "jungle night safari",
      "coffee and pepper estate walk",
      "jackfruit halwa"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Chembra Peak trek is one of the best day treks in South India — the heart-shaped lake at the top is genuinely one of a kind.",
      // "Wayanad is misty, green, and tribal in character — the coffee estates here feel more authentic than Coorg.",
      // "Roads to Wayanad through the Ghat sections are genuinely beautiful — don't rush this drive."
    // ]
  },
  "rameshwaram": {
    "city": "Rameshwaram",
    "lat": 9.2885,
    "lng": 79.31271,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "pilgrimage",
      "coastal",
      "monuments_museums"
    ],
    "must_visit": [
      "Ramanathaswamy Temple",
      "Dhanushkodi Ghost Town",
      "Pamban Bridge",
      "Adam's Bridge viewpoint",
      "Agni Theertham",
      "APJ Abdul Kalam Memorial"
    ],
    "must_try": [
      "22 theertham ritual bath (if you're up for it)",
      "sunrise at Agni Theertham beach",
      "fresh seafood at Dhanushkodi shacks",
      "Dhanushkodi ghost town walk at dawn",
      "Pamban rail bridge crossing by train",
      "pampering rice meals at local restaurants"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "auto",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Dhanushkodi is one of the eeriest and most beautiful places in India — a ruined town at land's end facing Sri Lanka.",
      // "The Pamban Bridge at dawn, with fishing boats and pink sky, is a genuinely iconic image.",
      // "The temple corridor is 1.2km — the longest in the world — and it's absolutely hypnotic to walk through."
    // ]
  },
  "madurai": {
    "city": "Madurai",
    "lat": 9.919,
    "lng": 78.11953,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "pilgrimage"
    ],
    "must_visit": [
      "Meenakshi Amman Temple",
      "Thirumalai Nayak Palace",
      "Gandhi Memorial Museum",
      "Koodal Azhagar Temple",
      "Vandiyur Mariamman Teppakulam",
      "Alagar Koil"
    ],
    "must_try": [
      "Meenakshi Temple corridor walk at night",
      "jigarthanda (local cooling drink)",
      "Madurai mutton biryani",
      "kari dosa at Murugan Idli Shop",
      "rooftop view of temple gopurams from nearby buildings",
      "parotta with Madurai-style salna",
      "jasmine garland buying at Nethaji market"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 5,
    // "notes": [
      // "Meenakshi Temple at night is other-worldly — the lit gopurams and the chanting create an atmosphere unlike anywhere else.",
      // "Jigarthanda is mandatory — it's a sweet, milky, nannari-flavoured cold drink unique to this city.",
      // "Madurai is hot, loud, and chaotic but has more concentrated temple architecture per square kilometre than almost anywhere."
    // ]
  },
  "guwahati": {
    "city": "Guwahati",
    "lat": 26.1844,
    "lng": 91.7458,
    "state": "assam",
    "region": "northeastern",
    "category": [
      "metro",
      "pilgrimage",
      "backpacker",
      "base_camp"
    ],
    "must_visit": [
      "Kamakhya Temple",
      "Umananda Island Temple",
      "Assam State Museum",
      "Srimanta Sankardev Kalakshetra",
      "Brahmaputra River Cruise",
      "Pobitora Wildlife Sanctuary (day trip)"
    ],
    "must_try": [
      "Assamese thali with masor tenga",
      "Pitha (rice cakes) from street stalls",
      "Ou tenga amlaboroi",
      "Duck meat curry",
      "Ferry ride across the Brahmaputra at dusk",
      "Shop for Mekhela Chador at Pan Bazaar"
    ],
    "accommodation": [
      "zostel"
    ],
    "mode_of_travel": [
      "city_bus",
      "auto",
      "ola/uber",
      "private_taxi"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Guwahati is the obligatory hub — use it wisely as a launchpad, not a destination.",
      // "Kamakhya before 7am is genuinely powerful; after 9am it's a crowd management exercise.",
      // "The Brahmaputra at dusk from the ferry is one of those quiet moments that resets you."
    // ]
  },
  "shillong": {
    "city": "Shillong",
    "lat": 25.56892,
    "lng": 91.88313,
    "state": "meghalaya",
    "region": "northeastern",
    "category": [
      "hill_station",
      "backpacker"
    ],
    "must_visit": [
      "Elephant Falls",
      "Shillong Peak",
      "Ward's Lake",
      "Don Bosco Museum",
      "Umiam Lake",
      "Laitlum Canyons"
    ],
    "must_try": [
      "Jadoh (rice and pork)",
      "Dohneiong (pork with black sesame)",
      "Tungrymbai",
      "Kwai (betel nut) culture observation",
      "Live music at Police Bazaar cafes",
      "Drive to Cherrapunji (day trip)",
      "Local craft shopping at Police Bazaar"
    ],
    "accommodation": [
      "zostel"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "private_taxi"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Shillong has a vibe that's entirely its own — part hill town, part rock city, part rainforest.",
      // "The cloud ceiling drops in the evening and suddenly you're inside a painting.",
      // "Eat pork without guilt here — it's genuinely a cultural experience."
    // ]
  },
  "dawki": {
    "city": "Dawki",
    "lat": 25.1969,
    "lng": 92.0183,
    "state": "meghalaya",
    "region": "northeastern",
    "category": [
      "backpacker",
      "border_town"
    ],
    "must_visit": [
      "Umngot River",
      "Dawki Suspension Bridge",
      "Bangladesh Border Zero Point",
      "Shnongpdeng Village",
      "Tyrna Village (starting point for Nongriat trek)"
    ],
    "must_try": [
      "Boat ride on the Umngot river (mandatory)",
      "Camp on the riverbank overnight",
      "Cliff jumping at Shnongpdeng",
      "Kayaking on Umngot",
      "Cross-border market browsing"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "bike_rental",
      "private_taxi"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 5,
    // "notes": [
      // "The Umngot river is so clear the boats look like they're floating on air — it genuinely looks photoshopped until you're there.",
      // "Come in November-December for the clearest water; monsoon turns it brown.",
      // "Shnongpdeng is where you stay if you want the real experience — not a hotel in Dawki town."
    // ]
  },
  "silchar": {
    "city": "Silchar",
    "lat": 24.82733,
    "lng": 92.79787,
    "state": "assam",
    "region": "northeastern",
    "category": [
      "backpacker",
      "monuments_museums",
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "Khaspur Ruins",
      "Maniharan Tunnel",
      "Bhubaneswari Temple",
      "Silchar Railway Station (historic)",
      "Barak River Ghat"
    ],
    "must_try": [
      "Barak Valley fish curry",
      "Shidol shutki (fermented fish chutney)",
      "Sour mustard greens with dal",
      "Boat ride on Barak river",
      "Evening walk at Silchar town market"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "private_taxi"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 2,
    // "notes": [
      // "Silchar is overlooked but has a quiet, unhurried Bengali-Assamese cultural blend that's genuinely interesting.",
      // "It's more of a base than a destination — but the food will catch you off guard.",
      // "The town runs at half-speed and that's its charm if you let yourself slow down."
    // ]
  },
  "dimapur": {
    "city": "Dimapur",
    "lat": 25.91174,
    "lng": 93.7217,
    "state": "nagaland",
    "region": "northeastern",
    "category": [
      "backpacker",
      "monuments_museums",
      "border_town",
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "Kachari Ruins",
      "Dimapur Ao Baptist Church",
      "Nagaland Zoological Park",
      "Triple Falls",
      "Herbs & Crafts Market"
    ],
    "must_try": [
      "Naga pork with bamboo shoot",
      "Smoked meat at local stalls",
      "Anishi (taro leaf dried)",
      "Try Naga chilli (Raja Mirchi) if you dare",
      "Explore Night Market for tribal crafts"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "private_taxi"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1 day",
    "rating": 2,
    // "notes": [
      // "Dimapur is Nagaland's ugly commercial face — a necessary stop, not a destination.",
      // "The Kachari ruins in the middle of the city are strangely haunting between all the traffic.",
      // "Stock up on smoked meats here before heading to Kohima — better variety and cheaper."
    // ]
  },
  "kohima": {
    "city": "Kohima",
    "lat": 25.67467,
    "lng": 94.11099,
    "state": "nagaland",
    "region": "northeastern",
    "category": [
      "monuments_museums",
      "backpacker"
    ],
    "must_visit": [
      "Kohima War Cemetery",
      "Nagaland State Museum",
      "Kisama Heritage Village",
      "Dzukou Valley",
      "Japfu Peak",
      "Kohima Cathedral"
    ],
    "must_try": [
      "Akini chokibo (steamed rice with vegetables)",
      "Fermented soybean (akhuni) dishes",
      "Galho (rice porridge)",
      "Naga tea (black with ginger and herbs)",
      "Hornbill Festival if visiting December",
      "Trek to Dzukou Valley"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "private_taxi"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Kohima War Cemetery is one of the most moving WWII sites in Asia — budget at least an hour of silence here.",
      // "The Hornbill Festival in December is one of India's most authentic cultural fests — worth planning a trip around.",
      // "Dzukou Valley feels like a different planet — do the overnight trek if your knees allow."
    // ]
  },
  "tinsukia": {
    "city": "Tinsukia",
    "lat": 27.48905,
    "lng": 95.35992,
    "state": "assam",
    "region": "northeastern",
    "category": [
      "backpacker"
    ],
    "must_visit": [
      "Dibru-Saikhowa National Park",
      "Maguri-Motapung Beel (wetland)",
      "Stilwell Road memorial",
      "Dehing Patkai Elephant Festival site",
      "Bordumsa Reserve Forest"
    ],
    "must_try": [
      "River dolphin watching at Maguri beel",
      "Jeep safari into Dibru-Saikhowa",
      "Feral horse watching (only place in India)",
      "Local Mishing tribe food",
      "Boat ride on the Brahmaputra tributaries"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "bike_rental",
      "private_taxi"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 3,
    // "notes": [
      // "Tinsukia is for people who mean business about wildlife — the feral horses of Dibru-Saikhowa are something out of a dream.",
      // "The wetlands at Maguri beel at dawn are quiet enough to hear your own heartbeat.",
      // "Don't come expecting tourist infrastructure — come expecting untouched Assam."
    // ]
  },
  "itanagar": {
    "city": "Itanagar",
    "lat": 27.08694,
    "lng": 93.60987,
    "state": "arunachal_pradesh",
    "region": "northeastern",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "backpacker"
    ],
    "must_visit": [
      "Ita Fort",
      "Jawaharlal Nehru State Museum",
      "Ganga Lake",
      "Buddhist Temple (Golden Pagoda)",
      "Namdapha National Park (accessible from here)"
    ],
    "must_try": [
      "Apong (rice beer)",
      "Bamboo shoot curry with pork",
      "Thukpa noodle soup",
      "Local tribal market at Naharlagun",
      "Hike around Ganga Lake at sunrise"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "private_taxi"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Itanagar requires an Inner Line Permit — get it sorted in advance or you'll waste your first day.",
      // "The state museum is genuinely excellent; more depth on Arunachali tribes than you'd expect.",
      // "It's a functional capital more than a tourist city, but the surrounding hills make up for it."
    // ]
  },
  "tawang": {
    "city": "Tawang",
    "lat": 27.57417,
    "lng": 91.92437,
    "state": "arunachal_pradesh",
    "region": "northeastern",
    "category": [
      "pilgrimage",
      "hill_station",
      "border_town"
    ],
    "must_visit": [
      "Tawang Monastery",
      "Sela Pass",
      "Madhuri Lake (Shungetser Lake)",
      "Bum La Pass",
      "Nuranang Waterfall",
      "War Memorial Tawang"
    ],
    "must_try": [
      "Butter tea (po cha)",
      "Thukpa with yak meat",
      "Cheese from local yak farms",
      "Drive over Sela Pass at 13,700 ft",
      "Visit Bum La Pass with army escort (permits needed)",
      "Evening prayer ceremony at Tawang Monastery"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "private_taxi"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Tawang Monastery at golden hour with the valley below is the kind of view that makes you reconsider everything.",
      // "The road via Sela Pass is terrifying and magnificent — go with a local driver who knows every blind curve.",
      // "Inner Line Permit and Protected Area Permit both needed — bureaucracy is annoying but the destination earns it."
    // ]
  },
  "darjeeling": {
    "city": "Darjeeling",
    "lat": 27.03333,
    "lng": 88.26667,
    "state": "west_bengal",
    "region": "eastern",
    "category": [
      "hill_station",
      "monuments_museums"
    ],
    "must_visit": [
      "Tiger Hill sunrise",
      "Batasia Loop",
      "Darjeeling Himalayan Railway (Toy Train)",
      "Happy Valley Tea Estate",
      "Padmaja Naidu Himalayan Zoological Park",
      "Peace Pagoda"
    ],
    "must_try": [
      "First flush Darjeeling tea at a tea garden",
      "Momos from Chowrasta",
      "Thukpa at local eateries",
      "Ride the Toy Train between Darjeeling and Ghoom",
      "Dawn drive to Tiger Hill for Kanchenjunga views",
      "Browse Chowrasta market for woolen goods"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "auto",
      "private_taxi"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "2-3 days",
    "rating": 5,
    // "notes": [
      // "Tiger Hill at 4am is a rite of passage — stand in the freezing dark and watch Kanchenjunga turn gold.",
      // "The Toy Train is slow, smoky, and absolutely perfect — do not skip it.",
      // "Darjeeling tea tastes different when you drink it at the estate where it's grown. That's not marketing, that's fact."
    // ]
  },
  "pelling": {
    "city": "Pelling",
    "lat": 27.2946,
    "lng": 88.6008,
    "state": "sikkim",
    "region": "northeastern",
    "category": [
      "hill_station",
      "pilgrimage"
    ],
    "must_visit": [
      "Pemayangtse Monastery",
      "Rabdentse Ruins",
      "Skywalk Pelling",
      "Singshore Bridge",
      "Kanchenjunga Falls",
      "Khecheopalri Lake"
    ],
    "must_try": [
      "Gundruk (fermented leafy greens)",
      "Chang (millet beer)",
      "Sel roti",
      "Dawn view of Kanchenjunga from your room window",
      "Skywalk over the valley",
      "Trek to Khecheopalri lake"
    ],
    "accommodation": [
      "zostel"
    ],
    "mode_of_travel": [
      "private_taxi"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Pelling has arguably the best unobstructed Kanchenjunga view in the region — Gangtok can't compete on this front.",
      // "The Skywalk is touristy but gives you a genuine perspective on the scale of the valleys below.",
      // "Khecheopalri lake has a serenity that's hard to explain — the locals say birds remove every fallen leaf from the surface."
    // ]
  },
  "gangtok": {
    "city": "Gangtok",
    "lat": 27.32574,
    "lng": 88.61216,
    "state": "sikkim",
    "region": "northeastern",
    "category": [
      "hill_station",
      "monuments_museums"
    ],
    "must_visit": [
      "Rumtek Monastery",
      "MG Marg",
      "Tsomgo Lake (Changu Lake)",
      "Nathu La Pass",
      "Enchey Monastery",
      "Namgyal Institute of Tibetology"
    ],
    "must_try": [
      "Phagshapa (pork with radish)",
      "Momos at MG Marg stalls",
      "Tongba (millet beer with bamboo straw)",
      "Chhurpi (hard yak cheese snack)",
      "Drive to Nathu La Pass at 14,140 ft",
      "Ropeway ride over the city"
    ],
    "accommodation": [
      "zostel",
      "moustache"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "private_taxi"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Gangtok is more polished than most hill stations — MG Marg at night feels almost European, which is either charming or uncanny.",
      // "Nathu La Pass requires a permit and often closes — check weather and book well in advance.",
      // "The monastery circuit here is understated; you can feel the Tibetan Buddhist influence without it being performance."
    // ]
  },
  "siliguri": {
    "city": "Siliguri",
    "lat": 26.71004,
    "lng": 88.42851,
    "state": "west_bengal",
    "region": "eastern",
    "category": [
      "backpacker",
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "North Bengal State Zoo (Bengal Safari)",
      "ISKCON Temple Siliguri",
      "Kali Mandir",
      "Hong Kong Market",
      "Coronation Bridge (Sevoke Bridge)"
    ],
    "must_try": [
      "Momo at Hong Kong Market",
      "Thukpa at Nepali dhabas",
      "Darjeeling tea shopping at bulk vendors",
      "Day trip to Jaldapara or Gorumara",
      "Night street food at Junction area"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1 day",
    "rating": 2,
    // "notes": [
      // "Siliguri is a junction city — treat it as a logistics hub, not a destination, and it'll serve you well.",
      // "The Bengal Safari is actually worth 3 hours if you have time to spare.",
      // "Hong Kong Market for cheap electronics and the momo stalls around it are the two reasons to actually stop here."
    // ]
  },
  "bengaluru": {
    "city": "Bengaluru",
    "lat": 12.97194,
    "lng": 77.59369,
    "state": "karnataka",
    "region": "southern",
    "category": [
      "metro",
      "base_camp"
    ],
    "must_visit": [
      "Cubbon Park",
      "Lalbagh Botanical Garden",
      "Bangalore Palace",
      "ISKCON Temple",
      "UB City",
      "Nandi Hills (day trip)"
    ],
    "must_try": [
      "masala dosa at MTR or Vidyarthi Bhavan",
      "craft beer at Toit or Arbor",
      "Bangalore breakfast trail",
      "Sunday antique market at Shivajinagar",
      "evening at Church Street",
      "filter coffee at Indian Coffee House",
      "biryani at Shivaji Military Hotel"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "the_hosteller",
      "bloom"
    ],
    "mode_of_travel": [
      "metro",
      "ola/uber",
      "auto",
      "city_bus"
    ],
    "ideal_season": "year_round",
    "ideal_stay": "3-4 days",
    "rating": 4,
    // "notes": [
      // "Bangalore is best experienced as a resident, not a tourist — the city's soul is in its neighbourhood cafés and pub culture.",
      // "Traffic is genuinely terrible; always use the metro for cross-city travel and save auto for last miles.",
      // "Nandi Hills at sunrise is a classic Bangalore escape — leave by 4:30am to beat the mist and the crowds."
    // ]
  },
  "kanchipuram": {
    "city": "Kanchipuram",
    "lat": 12.83515,
    "lng": 79.70006,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "monuments_museums",
      "pilgrimage"
    ],
    "must_visit": [
      "Kailasanathar Temple",
      "Ekambareswarar Temple",
      "Kamakshi Amman Temple",
      "Varadharaja Perumal Temple",
      "Vaikunta Perumal Temple",
      "Silk weaving workshops"
    ],
    "must_try": [
      "silk saree weaving unit visit",
      "Kailasanathar Temple sunrise visit",
      "Ekambareswarar mango tree (said to be 3500 years old)",
      "idli-sambar at local mess",
      "silk shopping directly from weavers",
      "Kamakshi Temple darshan"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "Kanchipuram's Kailasanathar Temple is the oldest in Tamil Nadu and one of the best-preserved Pallava structures — genuinely underrated.",
      // "The silk weaving units let you watch the process up close — buying directly here saves significantly over Chennai.",
      // "Can be done as a day trip from Chennai but staying overnight gives you the temples without tourist crowds."
    // ]
  },
  "tirupati": {
    "city": "Tirupati",
    "lat": 13.63551,
    "lng": 79.41989,
    "state": "andhra_pradesh",
    "region": "southern",
    "category": [
      "pilgrimage"
    ],
    "must_visit": [
      "Tirumala Venkateswara Temple",
      "Silathoranam (natural rock arch)",
      "Akasaganga Teertham",
      "Govindharaja Swamy Temple",
      "Sri Kapileswara Swamy Temple",
      "Chandragiri Fort"
    ],
    "must_try": [
      "Tirupati ladoo (prasadam)",
      "temple darshan (book special entry well in advance)",
      "hair tonsuring ritual if inclined",
      "Chandragiri Fort morning visit",
      "Tirupati local meals at Anna canteen",
      "dada hobbies bazaar shopping"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "city_bus",
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "The Tirumala temple at dawn with no crowd is a divine experience — book the Rs 300 special darshan online a month in advance.",
      // "Tirupati ladoos are genuinely different from reproductions — get them fresh from the official counters only.",
      // "The serpentine queue for general darshan can be 18+ hours; plan smart unless you have special entry."
    // ]
  },
  "srisailam": {
    "city": "Srisailam",
    "lat": 16.07217,
    "lng": 78.86816,
    "state": "andhra_pradesh",
    "region": "southern",
    "category": [
      "pilgrimage"
    ],
    "must_visit": [
      "Mallikarjuna Jyotirlinga Temple",
      "Nagarjuna Sagar Dam viewpoint",
      "Srisailam Wildlife Sanctuary",
      "Shakti Peetha (Bhramaramba Devi)",
      "Patal Ganga Ropeway",
      "Srisailam Dam"
    ],
    "must_try": [
      "Mallikarjuna lingam abhishekam darshan",
      "ropeway ride to Patal Ganga on Krishna river",
      "jungle safari through Nagarjunasagar Srisailam Tiger Reserve",
      "village-style Andhra thali",
      "dam viewpoint at sunrise",
      "trek through the Nallamala forest"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Srisailam combines a Jyotirlinga with a massive dam and dense tiger forest — one of the most underrated religious-nature combos in India.",
      // "The ropeway over the Krishna gorge is vertiginous and spectacular — don't skip it.",
      // "The roads through Nallamala forest after dark can be risky; plan to arrive before sunset."
    // ]
  },
  "hyderabad": {
    "city": "Hyderabad",
    "lat": 17.38405,
    "lng": 78.45636,
    "state": "telangana",
    "region": "southern",
    "category": [
      "metro",
      "forts_palaces",
      "monuments_museums",
      "base_camp"
    ],
    "must_visit": [
      "Charminar",
      "Golconda Fort",
      "Hussain Sagar Lake",
      "Qutb Shahi Tombs",
      "Salar Jung Museum",
      "Ramoji Film City"
    ],
    "must_try": [
      "Hyderabadi dum biryani at Paradise or Bawarchi",
      "Irani chai with Osmania biscuits",
      "haleem (especially during Ramadan)",
      "Charminar laad bazaar bangle shopping",
      "Golconda Fort light and sound show",
      "paya and nihari at Old City",
      "Karachi Bakery fruit biscuits"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller",
      "bloom"
    ],
    "mode_of_travel": [
      "metro",
      "ola/uber",
      "auto",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Hyderabadi biryani is not a tourist dish — it's a cultural institution, and every local has a sworn loyalty to one shop.",
      // "Golconda Fort's acoustics test is genuinely clever; a clap at the entrance gate can be heard at the top of the fort.",
      // "Old Hyderabad near Charminar is a completely different world from Hitech City — don't leave without spending time in the bazaars."
    // ]
  },
  "shivamogga": {
    "city": "Shivamogga",
    "lat": 13.93157,
    "lng": 75.56791,
    "state": "karnataka",
    "region": "southern",
    "category": [
      "forts_palaces"
    ],
    "must_visit": [
      "Jog Falls",
      "Sharavathi Wildlife Sanctuary",
      "Tyavarekoppa Lion & Tiger Reserve",
      "Keladi Temple",
      "Shivappa Nayaka Palace",
      "Kodachadri Peak (via Hosanagara)"
    ],
    "must_try": [
      "Jog Falls viewpoint trek (walk down to base)",
      "river rafting on Sharavathi",
      "Kodachadri peak trek overnight",
      "local akki rotti with chicken",
      "arecanut plantation walks",
      "fishing villages along Linganamakki reservoir"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "monsoon",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Jog Falls in monsoon is one of the most powerful natural sights in India — the force of water is humbling.",
      // "Most people visit Jog Falls and leave the same day; staying in Shivamogga lets you explore the wildlife sanctuary and Kodachadri.",
      // "Shivamogga is a classic underrated Karnataka gem — lush, cheap, and barely touristy."
    // ]
  },
  "pondicherry": {
    "city": "Pondicherry",
    "lat": 11.93381,
    "lng": 79.82979,
    "state": "pondicherry",
    "region": "southern",
    "category": [
      "coastal",
      "monuments_museums",
      "backpacker"
    ],
    "must_visit": [
      "French Quarter (White Town)",
      "Auroville",
      "Sri Aurobindo Ashram",
      "Promenade Beach",
      "Basilica of the Sacred Heart of Jesus",
      "Paradise Beach"
    ],
    "must_try": [
      "French bakery breakfast in White Town",
      "crêpes and espresso at Café des Arts",
      "Auroville Matrimandir meditation (book ahead)",
      "sunrise at Promenade Beach",
      "cycling White Town streets at 7am before traffic",
      "fresh seafood at Seagulls restaurant",
      "Pondicherry-style fish curry"
    ],
    "accommodation": [
      "zostel",
      "gostops"
    ],
    "mode_of_travel": [
      "bike_rental",
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "White Town at dawn on a rented bicycle is one of my favourite things to do in South India — bougainvillea walls and French villas before the scooters arrive.",
      // "Auroville Matrimandir requires advance registration — don't show up without it expecting to get in.",
      // "The French Quarter is genuinely pleasant but gets overrun by Chennai weekenders on Saturdays — arrive Sunday morning."
    // ]
  },
  "chennai": {
    "city": "Chennai",
    "lat": 13.08784,
    "lng": 80.27847,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "metro",
      "coastal",
      "forts_palaces",
      "monuments_museums",
      "base_camp"
    ],
    "must_visit": [
      "Marina Beach",
      "Kapaleeshwarar Temple",
      "Government Museum & Egmore",
      "Fort St. George",
      "Mahabalipuram (day trip)",
      "San Thome Basilica"
    ],
    "must_try": [
      "chettinad chicken curry at Anjappar",
      "filter coffee at Saravana Bhavan",
      "Marina Beach sundal (boiled chickpeas snack)",
      "Mylapore street walk at dawn",
      "Parotta and egg curry at local mess",
      "Besant Nagar beach evening",
      "Kothu parotta at 1am"
    ],
    "accommodation": [
      "zostel",
      "bloom"
    ],
    "mode_of_travel": [
      "metro",
      "city_bus",
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Chennai doesn't show off — it's a working city, and you have to seek out its pleasures rather than have them presented.",
      // "The Mylapore neighbourhood around Kapaleeshwarar Temple is the real Chennai — flower sellers, silk saree shops, and kolam drawings on every doorstep.",
      // "Marina Beach at 6am is serene; by 9am it's one of the most densely packed beaches in the world."
    // ]
  },
  "sonmarg": {
    "city": "Sonmarg",
    "lat": 34.3027,
    "lng": 75.2948,
    "state": "jammu_kashmir",
    "region": "northern",
    "category": [
      "hill_station"
    ],
    "must_visit": [
      "Thajiwas Glacier",
      "Vishansar Lake trek",
      "Krishnasar Lake",
      "Zoji La Pass",
      "Baltal side valley"
    ],
    "must_try": [
      "Glacial snow activities at Thajiwas",
      "Trout from Sindh river",
      "Noon chai in meadow camps",
      "Pony ride to the glacier",
      "Camping in the high meadows",
      "Drive towards Zoji La for sunrise views"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [],
    "ideal_season": "summer",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "Sonmarg's meadows in June when the snow is still on the peaks and the wildflowers are out is as beautiful as Kashmir gets.",
      // "The road to Zoji La is hair-raising but worth it for the views — only attempt in good weather.",
      // "Thajiwas Glacier is just a 2km walk from the main meadow — you don't need a pony unless you want one."
    // ]
  },
  "baltal": {
    "city": "Baltal",
    "lat": 34.2139,
    "lng": 75.4767,
    "state": "jammu_kashmir",
    "region": "northern",
    "category": [
      "pilgrimage"
    ],
    "must_visit": [
      "Amarnath Cave Temple (trek from Baltal)",
      "Baltal base camp",
      "Sindh river crossing",
      "Zoji La pass view",
      "High altitude meadow walks"
    ],
    "must_try": [
      "Amarnath Yatra darshan (if in season)",
      "High altitude langar at base camp",
      "Glacial stream crossing experience",
      "Watching the yatra procession from the valley",
      "Camping at 9,000 ft under stars"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [],
    "ideal_season": "summer",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Baltal is the shorter and steeper route to Amarnath — 14km versus 36km from Pahalgam, but more demanding.",
      // "The Yatra season is July-August only — outside that window Baltal is just a beautiful but basic camp site.",
      // "Physical fitness and acclimatization matter here — altitude and gradient together catch many people off guard."
    // ]
  },
  "banihal-udhampur": {
    "city": "Udhampur",
    "lat": 33.43647,
    "lng": 75.19684,
    "state": "jammu_kashmir",
    "region": "northern",
    "category": [],
    "must_visit": [
      "Banihal Railway Station (highest broad-gauge section)",
      "Patnitop Hill Station",
      "Sudh Mahadev Temple",
      "Nathatop viewpoint",
      "Chenab Bridge viewpoint (world's highest railway bridge)"
    ],
    "must_try": [
      "Rajma chawal at Patnitop dhabas",
      "Snow at Patnitop in winter",
      "Hot pakoras at highway stalls",
      "Viewpoint stop at Chenab gorge",
      "Drive through the Banihal-Udhampur mountain stretch at dawn"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [],
    "ideal_season": "summer",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "The Chenab Railway Bridge under construction (now complete) is an engineering marvel — worth a dedicated stop.",
      // "Patnitop is a pleasant, undiscovered Himalayan meadow town that deserves more attention than it gets.",
      // "This corridor has transformed with the Udhampur-Srinagar-Baramulla Rail Link — train travel here is now possible."
    // ]
  },
  "orchha": {
    "city": "Orchha",
    "lat": 25.35192,
    "lng": 78.64033,
    "state": "madhya_pradesh",
    "region": "central",
    "category": [
      "forts_palaces",
      "monuments_museums"
    ],
    "must_visit": [
      "Orchha Fort Complex",
      "Raja Ram Temple",
      "Chaturbhuj Temple",
      "Laxminarayan Temple (murals)",
      "Jahangir Mahal",
      "Orchha cenotaphs on Betwa river"
    ],
    "must_try": [
      "Peda at the temple",
      "Betwa river boat ride",
      "Sunrise at the cenotaphs",
      "Cycling between temples",
      "Sunset from the fort ramparts"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 5,
    // "notes": [
      // "Orchha is one of those rare places that feels completely frozen in time — 17th-century cenotaphs standing in the river with vultures overhead and almost no other tourists.",
      // "The Raja Ram Temple is the only place in India where Lord Ram is worshipped as a king rather than a god and carries a unique military tradition — fascinating.",
      // "Stay overnight to get the town to yourself after the day-trippers leave — the morning light on the Betwa ghats is extraordinary."
    // ]
  },
  "jhansi": {
    "city": "Jhansi",
    "lat": 25.45887,
    "lng": 78.57994,
    "state": "madhya_pradesh",
    "region": "central",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "in_transit"
    ],
    "must_visit": [
      "Jhansi Fort",
      "Rani Mahal",
      "Government Museum",
      "Shiv Mandir Khandagiri",
      "St. Jude's Shrine"
    ],
    "must_try": [
      "Bedai-kachori breakfast",
      "Tehri (saffron rice)",
      "Jhansi ki Rani son et lumière show",
      "Local bazaar walk",
      "Day trip to Orchha"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Jhansi is primarily a gateway city rather than a destination but the fort carries genuine weight — standing where Lakshmibai made her last stand is powerful.",
      // "Most people rush through on the way to Orchha or Khajuraho but the Government Museum has an excellent collection of local sculpture worth an hour.",
      // "The town's Bundeli food — particularly the tehri and bedai — is distinct from both UP and MP cuisines and should not be missed."
    // ]
  },
  "khajurao": {
    "city": "Khajurao",
    "lat": 24.8318,
    "lng": 79.9229,
    "state": "madhya_pradesh",
    "region": "central",
    "category": [
      "monuments_museums"
    ],
    "must_visit": [
      "Western Temple Group (Kandariya Mahadeva)",
      "Eastern Temple Group",
      "Southern Group",
      "Archaeological Museum",
      "Raneh Falls (day trip)",
      "Ken Gharial Sanctuary"
    ],
    "must_try": [
      "Sound and light show at Western Temples",
      "Cycling between temple groups",
      "Sunrise at Kandariya Mahadeva",
      "Tribal art shopping",
      "Local thali at dhabas"
    ],
    "accommodation": [
      "moustache",
      "madpackers"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 5,
    // "notes": [
      // "The sculptures here are not what the internet's obsession with the erotic panels suggests — the temple complexes are among the finest examples of Chandela architecture and the cosmic iconography is genuinely complex.",
      // "Most tourists do a three-hour walk and leave — rent a cycle, slow down, and spend a full day in the western group alone, it rewards patience enormously.",
      // "Raneh Falls nearby — red and grey volcanic gorge cut by the Ken river — is one of Madhya Pradesh's best kept natural secrets."
    // ]
  },
  "prayagraj": {
    "city": "Prayagraj",
    "lat": 25.4358,
    "lng": 81.8463,
    "state": "uttar_pradesh",
    "region": "northern",
    "category": [
      "pilgrimage",
      "forts_palaces",
      "monuments_museums"
    ],
    "must_visit": [
      "Triveni Sangam",
      "Allahabad Fort",
      "Anand Bhavan",
      "Khusro Bagh",
      "Hanuman Mandir (inverted idol)"
    ],
    "must_try": [
      "Triveni Sangam holy dip",
      "Tehri and kachori at Civil Lines",
      "Aloo tikki chaat near Sangam",
      "Chai at old bazaar stalls",
      "Maha Kumbh experience (if timing allows)",
      "Boat ride to Sangam confluence"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "The Sangam at dawn when the mist sits on the water feels genuinely sacred regardless of belief.",
      // "Kumbh Mela here is once in a lifetime if you can handle the scale — otherwise the city is a calm two-day stop.",
      // "Anand Bhavan gives you a real window into the freedom movement that textbooks miss."
    // ]
  },
  "ayodhya": {
    "city": "Ayodhya",
    "lat": 26.79909,
    "lng": 82.2047,
    "state": "uttar_pradesh",
    "region": "northern",
    "category": [
      "pilgrimage"
    ],
    "must_visit": [
      "Ram Mandir",
      "Hanuman Garhi",
      "Kanak Bhawan",
      "Saryu Ghat",
      "Ram Ki Paidi"
    ],
    "must_try": [
      "Evening aarti at Saryu Ghat",
      "Prasad at Ram Mandir",
      "Peda and ladoo from local sweet shops",
      "Boat ride on Saryu river",
      "Chaat near Hanuman Garhi steps",
      "Parikrama of the main temple complex"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "The new Ram Mandir is architecturally stunning — the scale of devotion around it is something you can only feel in person.",
      // "Saryu Ghat at dusk with the aarti is a quieter, more personal version of the Varanasi experience.",
      // "Crowds during Ram Navami and Diwali are immense — plan accordingly or avoid peak days."
    // ]
  },
  "lukhnow": {
    "city": "Lukhnow",
    "lat": 26.83928,
    "lng": 80.92313,
    "state": "uttar_pradesh",
    "region": "northern",
    "category": [
      "monuments_museums"
    ],
    "must_visit": [
      "Bara Imambara",
      "Chota Imambara",
      "Rumi Darwaza",
      "Hazratganj",
      "Residency Ruins"
    ],
    "must_try": [
      "Tunday Kababi galouti kebab",
      "Lucknowi biryani at Wahid or Idris",
      "Kulfi at Prakash Kulfi",
      "Basket chaat at Chowk",
      "Sheermal and malaai gilori paan",
      "Walking tour of Aminabad bazaar"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "The food alone justifies the trip — Tunde Kababi is not hype, it genuinely melts.",
      // "Bara Imambara's bhul-bhulaiya maze is one of the most underrated architectural experiences in India.",
      // "Lucknow has a slowness and grace to it that most north Indian cities have lost — lean into it."
    // ]
  },
  "bodh-gaya": {
    "city": "Bodh Gaya",
    "lat": 24.69808,
    "lng": 84.9869,
    "state": "bihar",
    "region": "eastern",
    "category": [
      "pilgrimage",
      "monuments_museums",
      "backpacker"
    ],
    "must_visit": [
      "Mahabodhi Temple",
      "Bodhi Tree",
      "Great Buddha Statue",
      "Thai Monastery",
      "Japanese Temple",
      "Dungeshwari Cave (Sujata Stupa)"
    ],
    "must_try": [
      "Sit under the Bodhi Tree in meditation (even if you're not Buddhist)",
      "Visit Mahabodhi at 5am for the morning prayer",
      "Tibetan food at monasteries",
      "Witness the international Buddhist prayer congregation",
      "Walk the Chakramarana (walking meditation path)",
      "Bodhgaya Market for Buddhist artifacts"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "The Mahabodhi Temple complex at dawn, with monks chanting from every direction, is genuinely transcendent.",
      // "Buddhists from Thailand, Japan, Sri Lanka, Tibet all share this space — the diversity of devotion is the attraction.",
      // "It's a pilgrimage site, not a tourist attraction — carry that energy into how you move through it."
    // ]
  },
  "deoghar": {
    "city": "Deoghar",
    "lat": 24.4802,
    "lng": 86.6919,
    "state": "jharkhand",
    "region": "eastern",
    "category": [
      "pilgrimage"
    ],
    "must_visit": [
      "Baidyanath Dham Temple (Jyotirlinga)",
      "Basukinath Temple",
      "Nandan Pahar",
      "Tapovan Cave",
      "Trikuta Hills Ropeway",
      "Satsang Ashram"
    ],
    "must_try": [
      "Prasad from temple (Pedha)",
      "Shravani Mela if visiting during August",
      "Evening Aarti at Baidyanath Dham",
      "Litti chokha",
      "Local peda sweets from Deoghar halwais"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Baidyanath Dham is one of the twelve Jyotirlingas — during Shravani Mela, millions walk hundreds of kilometers to offer water here.",
      // "The intensity of devotion here is unlike anything you'll see at more 'accessible' temples.",
      // "Outside of Shravani Mela it's quieter and more manageable — you can actually touch the sanctum."
    // ]
  },
  "digha": {
    "city": "Digha",
    "lat": 21.62776,
    "lng": 87.51965,
    "state": "west_bengal",
    "region": "eastern",
    "category": [
      "coastal",
      "backpacker"
    ],
    "must_visit": [
      "Old Digha Beach",
      "New Digha Beach",
      "Amarabati Park",
      "Marine Aquarium & Research Centre",
      "Talsari Beach (Odisha border)"
    ],
    "must_try": [
      "Fresh prawn and crab at beach-side stalls",
      "Fried hilsa if in season",
      "Walk the beach at dawn when it's empty",
      "Sunset at New Digha with local tea",
      "Day trip to Shankarpur or Mandarmani"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "auto",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 2,
    // "notes": [
      // "Digha is the Bengalis' goa — affordable, accessible, and perpetually crowded on weekends.",
      // "Go on a weekday or it's just wall-to-wall people from Kolkata with speakers playing Bengali film songs.",
      // "The seafood is the actual reason to come — ignore the over-developed beach and focus on the food."
    // ]
  },
  "kharagpur": {
    "city": "Kharagpur",
    "lat": 22.3297,
    "lng": 87.3236,
    "state": "west_bengal",
    "region": "eastern",
    "category": [
      "backpacker",
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "IIT Kharagpur Campus",
      "Kharagpur Railway Station (longest platform in India)",
      "Hijli Detention Camp",
      "Nehru Museum of Science & Technology",
      "Balasore-Kharagpur expressway stretch"
    ],
    "must_try": [
      "Eat at the IIT kumbha (mess) if you know someone inside",
      "Platform tea at Kharagpur station",
      "Matir bhande mutton from local dhabas",
      "Observe the engineering student culture",
      "Visit the Hijli Campus heritage buildings"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1 day",
    "rating": 2,
    // "notes": [
      // "Kharagpur is not a tourist destination — it's a university town that happens to have India's longest railway platform.",
      // "The IIT campus is genuinely beautiful if you can get a pass in — colonial bungalows and tree-lined avenues.",
      // "A useful overnight break on the Kolkata-Bhubaneswar road trip rather than a standalone trip."
    // ]
  },
  "cuttak": {
    "city": "Cuttak",
    "lat": 20.46497,
    "lng": 85.87927,
    "state": "odisha",
    "region": "eastern",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "pilgrimage",
      "in_transit"
    ],
    "must_visit": [
      "Barabati Fort",
      "Kadam Rasul Mosque",
      "Cuttack Chandi Temple",
      "Mahanadi Riverfront",
      "Silver Filigree workshops (Tarakasi)",
      "Netaji Birth Place Museum"
    ],
    "must_try": [
      "Dahibara Aludum (street food classic)",
      "Rasabali",
      "Cuttack Chaat at the lane near Balu Bazaar",
      "Silver filigree jewelry workshop visit",
      "Bara ghanta (taro curry)",
      "Dusk walk along Mahanadi ghats"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "city_bus",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Cuttack's silver filigree craft is some of the finest in the world — a workshop visit is genuinely mind-blowing.",
      // "Dahibara Aludum at the roadside stalls is the kind of street food that ruins restaurant versions for you.",
      // "Netaji Subhas Chandra Bose was born here — the museum is small but sincere."
    // ]
  },
  "vishakhapatnam": {
    "city": "Vishakhapatnam",
    "lat": 17.68009,
    "lng": 83.20161,
    "state": "andhra_pradesh",
    "region": "eastern",
    "category": [
      "coastal",
      "metro",
      "monuments_museums"
    ],
    "must_visit": [
      "RK Beach",
      "Kailasagiri Hill Park",
      "Submarine Museum (INS Kursura)",
      "Araku Valley (day trip)",
      "Borra Caves",
      "VUDA Park"
    ],
    "must_try": [
      "Pesarattu with ginger chutney",
      "Royyala (prawn) curry at beach restaurants",
      "Fresh-caught fish at Jodugullapalem fish market",
      "Drive to Araku Valley on the Ghat Road",
      "Borra cave exploration",
      "Vizag Biryani (different from Hyderabadi)"
    ],
    "accommodation": [
      "zostel"
    ],
    "mode_of_travel": [
      "city_bus",
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Vizag is an underrated coastal city — less chaotic than Chennai, less expensive than Goa, and genuinely beautiful.",
      // "The Araku Valley day trip is mandatory — the ghat road alone is worth it.",
      // "INS Kursura submarine museum is one of the few places you can go inside a decommissioned navy submarine — claustrophobic but fascinating."
    // ]
  },
  "araku": {
    "city": "Araku",
    "lat": 18.327,
    "lng": 82.8756,
    "state": "andhra_pradesh",
    "region": "southern",
    "category": [
      "hill_station",
      "monuments_museums"
    ],
    "must_visit": [
      "Borra Caves",
      "Araku Valley Coffee Plantations",
      "Tribal Museum (ITDA)",
      "Chaparai Water Cascades",
      "Padmapuram Gardens",
      "Galikonda View Point"
    ],
    "must_try": [
      "Araku valley coffee (buy directly from tribal cooperative)",
      "train journey from Visakhapatnam (one of the most scenic rail routes in India)",
      "tribal village walk",
      "bamboo chicken (chicken cooked inside bamboo)",
      "Borra Caves stalactite tour",
      "local tribal millet dishes"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "The Kirandul Express train through the Eastern Ghats to Araku is genuinely one of the most scenic rail journeys in India — window seat mandatory.",
      // "Borra Caves are much larger and more impressive than I expected — the limestone formations are extraordinary.",
      // "Araku coffee is excellent and dirt cheap at the source; the tribal cooperatives sell it at a fraction of what Chennai shops charge."
    // ]
  },
  "coorg": {
    "city": "Coorg",
    "lat": 12.42602,
    "lng": 75.7382,
    "state": "karnataka",
    "region": "southern",
    "category": [
      "hill_station"
    ],
    "must_visit": [
      "Abbey Falls",
      "Raja's Seat",
      "Talakaveri (Kaveri River Source)",
      "Namdroling Monastery (Golden Temple)",
      "Dubare Elephant Camp",
      "Tadiandamol Peak"
    ],
    "must_try": [
      "Coorg pork curry with akki rotti",
      "coffee estate walk and tasting",
      "elephant bathing at Dubare camp",
      "Tadiandamol trek (highest peak in Coorg)",
      "Kodava-style homestay dinner",
      "locally grown filter coffee"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "the_hosteller"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Coorg is best enjoyed at a homestay run by a Kodava family — the food, hospitality, and local knowledge are unmatched.",
      // "The Namdroling Golden Temple is surprisingly spectacular — gold-lacquered Tibetan Buddhist architecture in the middle of coffee country.",
      // "Monsoon Coorg is lush but the roads get bad; October-March is the sweet spot."
    // ]
  },
  "chikkmanglore": {
    "city": "Chikkmanglore",
    "lat": 13.32231,
    "lng": 75.774,
    "state": "karnataka",
    "region": "southern",
    "category": [
      "hill_station"
    ],
    "must_visit": [
      "Mullayanagiri Peak",
      "Baba Budangiri",
      "Kudremukh National Park",
      "Hebbe Falls",
      "Kemmanagundi Hill Station",
      "Bhadra Wildlife Sanctuary"
    ],
    "must_try": [
      "Mullayanagiri peak trek (highest in Karnataka)",
      "Kudremukh jungle trek (requires forest permit)",
      "local filter coffee at Chikmagalur town",
      "pepper and cardamom estate tour",
      "night stay at coffee estate homestay",
      "kadubu (steamed rice dumpling) breakfast"
    ],
    "accommodation": [
      "the_hosteller"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Mullayanagiri at dawn is magnificent — you're above the clouds and can see all the way to the coast on a clear day.",
      // "Chikmagalur is overrun by Bangaloreans on weekends; plan for weekdays if you want the quiet coffee estate vibe.",
      // "Kudremukh requires advance forest department permits but is one of the most pristine treks in the Western Ghats."
    // ]
  },
  "hampi": {
    "city": "Hampi",
    "lat": 15.3352,
    "lng": 76.4603,
    "state": "karnataka",
    "region": "southern",
    "category": [
      "monuments_museums",
      "backpacker"
    ],
    "must_visit": [
      "Virupaksha Temple",
      "Vittala Temple & Stone Chariot",
      "Matanga Hill (sunrise point)",
      "Hampi Bazaar ruins",
      "Elephant Stables",
      "Hemakuta Hill"
    ],
    "must_try": [
      "sunrise on Matanga Hill",
      "coracle ride across Tungabhadra river to Hippie Island",
      "boulder scrambling in the ruins",
      "banana pancake and chai at Virupapur Gadde guesthouses",
      "evening light on Vittala Temple",
      "bicycle through the ruins at dusk",
      "Tungabhadra river sunset"
    ],
    "accommodation": [
      "zostel"
    ],
    "mode_of_travel": [
      "bike_rental",
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Hampi is possibly the most atmospheric place in South India — 500-year-old Vijayanagara empire ruins scattered across a lunar boulder landscape.",
      // "The coracle ferry to Hippie Island is the best Rs 20 you'll spend in India — the other side has a completely different backpacker energy.",
      // "Hire a bicycle, not an auto — the ruins are spread out and half the joy is cycling through paddy fields between temples."
    // ]
  },
  "mudurai": {
    "city": "Mudurai",
    "lat": 9.919,
    "lng": 78.11953,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "pilgrimage"
    ],
    "must_visit": [
      "Meenakshi Amman Temple",
      "Thirumalai Nayak Palace",
      "Gandhi Memorial Museum",
      "Koodal Azhagar Temple",
      "Vandiyur Mariamman Teppakulam",
      "Alagar Koil"
    ],
    "must_try": [
      "Meenakshi Temple corridor walk at night",
      "jigarthanda (local cooling drink)",
      "Madurai mutton biryani",
      "kari dosa at Murugan Idli Shop",
      "rooftop view of temple gopurams from nearby buildings",
      "parotta with Madurai-style salna",
      "jasmine garland buying at Nethaji market"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 5,
    // "notes": [
      // "Meenakshi Temple at night is other-worldly — the lit gopurams and the chanting create an atmosphere unlike anywhere else.",
      // "Jigarthanda is mandatory — it's a sweet, milky, nannari-flavoured cold drink unique to this city.",
      // "Madurai is hot, loud, and chaotic but has more concentrated temple architecture per square kilometre than almost anywhere."
    // ]
  },
  "kodaikannal": {
    "city": "Kodaikannal",
    "lat": 10.2381,
    "lng": 77.4892,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "hill_station",
      "backpacker"
    ],
    "must_visit": [
      "Kodaikanal Lake",
      "Coaker's Walk",
      "Bryant Park",
      "Pillar Rocks",
      "Green Valley View (Suicide Point)",
      "Bear Shola Falls"
    ],
    "must_try": [
      "boat ride on Kodaikanal Lake",
      "Coaker's Walk at sunrise in the mist",
      "fresh eucalyptus oil and homemade chocolate shopping",
      "horse riding around the lake",
      "Pillar Rocks viewpoint at sunset",
      "Kodaikanal Special Porridge at local restaurants"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller"
    ],
    "mode_of_travel": [
      "bike_rental",
      "auto",
      "private_taxi"
    ],
    "ideal_season": "summer",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Kodaikanal is the cooler, quieter cousin of Ooty — same colonial hill-station charm with better forest trails.",
      // "Coaker's Walk on a clear morning gives views all the way to the plains — worth waking up at 6am for.",
      // "The local chocolate shops are tourist traps; the homemade stuff from tiny stalls near the lake is genuinely good and cheap."
    // ]
  },
  "poombarai": {
    "city": "Poombarai",
    "lat": 9.84745,
    "lng": 77.29603,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "hill_station",
      "backpacker"
    ],
    "must_visit": [
      "Poombarai Village viewpoint",
      "Pambar River banks",
      "Kurinji Andavar Temple",
      "Berijam Lake (permit required)",
      "Perumal Peak trails",
      "Old terraced shola forests"
    ],
    "must_try": [
      "offbeat trek through cardamom and coffee estates",
      "sunrise from Poombarai hilltop terrace",
      "local tribal home cooking",
      "Berijam Lake morning walk (Forest Dept permit)",
      "cycling through the shola grassland edges",
      "fresh cardamom chai from village shops"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Poombarai is a genuinely hidden gem above Kodaikanal — most tourists never come this far, which is precisely the point.",
      // "The terraced vegetable gardens against the misty backdrop look like a painting — best light in the early morning.",
      // "Almost zero tourist infrastructure; carry food and water and book accommodation well in advance."
    // ]
  },
  "theni": {
    "city": "Theni",
    "lat": 10.01115,
    "lng": 77.47772,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "hill_station",
      "in_transit"
    ],
    "must_visit": [
      "Megamalai Wildlife Sanctuary",
      "High Wavy Mountains",
      "Suruli Falls",
      "Kumbakkarai Falls",
      "Bodi Mettu viewpoint",
      "Vaigai River origin"
    ],
    "must_try": [
      "Megamalai tea estate walk",
      "cardamom and pepper plantation visits",
      "Suruli Falls swimming",
      "local Theni biryani (distinct Chettinad-adjacent style)",
      "Bodinayakkanur grapes (it's the grape capital of TN)",
      "jungle trekking permits through forest department"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "bike_rental",
      "private_taxi"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Theni is primarily a base for Megamalai — one of Tamil Nadu's most beautiful and least visited wildlife sanctuaries.",
      // "Bodinayakkanur is the grape capital of Tamil Nadu; the vineyards and fruit markets here are a genuine surprise.",
      // "Most people rush through Theni on their way to Munnar or Kodaikanal; spending a night here rewards you with serene, unhurried access to the hills."
    // ]
  },
  "munnar": {
    "city": "Munnar",
    "lat": 10.08818,
    "lng": 77.06239,
    "state": "kerala",
    "region": "southern",
    "category": [
      "hill_station"
    ],
    "must_visit": [
      "Eravikulam National Park (Nilgiri Tahr)",
      "Top Station viewpoint",
      "Mattupetty Dam",
      "Anamudi Peak (highest in South India)",
      "Tea Museum",
      "Attukal Waterfalls"
    ],
    "must_try": [
      "sunrise at Top Station",
      "Eravikulam National Park tahr sighting",
      "tea factory tour and tasting",
      "Munnar tea-estate cycling",
      "echo point boat ride",
      "Munnar special tea (cardamom and ginger blend)",
      "camping at Rajamala"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller"
    ],
    "mode_of_travel": [
      "bike_rental",
      "private_taxi"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Munnar's tea estates in the morning mist are iconic for a reason — the green-on-green landscape is endlessly photogenic.",
      // "Eravikulam Park is the only place in the world to see the Nilgiri Tahr; go early for best sightings.",
      // "Munnar is crowded on weekends year-round; book accommodation early and arrive midweek for the best experience."
    // ]
  },
  "thrissur": {
    "city": "Thrissur",
    "lat": 10.51667,
    "lng": 76.21667,
    "state": "kerala",
    "region": "southern",
    "category": [
      "monuments_museums",
      "pilgrimage"
    ],
    "must_visit": [
      "Vadakkunnathan Temple",
      "Thrissur Pooram venue (Thekkinkadu Maidan)",
      "Guruvayur Temple (30km)",
      "Kerala Sangeetha Nataka Academy",
      "Archaeological Museum",
      "Athirapally Waterfalls (60km)"
    ],
    "must_try": [
      "Thrissur Pooram festival (May)",
      "Guruvayur Temple darshan and elephants",
      "Athirapally Falls morning visit",
      "puttu-kadala curry breakfast at any local mess",
      "Kerala traditional mural art class",
      "Shakkara Payasam (sweet rice dessert at temples)"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "city_bus",
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "If you are here during Thrissur Pooram, cancel all plans and stay — it's the most spectacular elephant festival in the world.",
      // "Athirapally waterfall is worth the 60km trip, especially post-monsoon when the water volume is massive.",
      // "Thrissur is Kerala's cultural capital — more traditional and Brahminical than Kochi, which gives it a different energy."
    // ]
  },
  "palakkad": {
    "city": "Palakkad",
    "lat": 10.7867,
    "lng": 76.6548,
    "state": "kerala",
    "region": "southern",
    "category": [
      "forts_palaces",
      "in_transit"
    ],
    "must_visit": [
      "Palakkad Fort",
      "Silent Valley National Park",
      "Malampuzha Dam & Gardens",
      "Nelliyampathy Hills",
      "Dhoni Waterfalls",
      "Parambikulam Tiger Reserve"
    ],
    "must_try": [
      "Silent Valley National Park trek (permit required)",
      "Nelliyampathy sunrise viewpoint",
      "Parambikulam jungle safari",
      "Palakkad-style Iyer cuisine (distinct Tamil-Brahmin food)",
      "Malampuzha rope-way ride",
      "fresh organic mangoes (Palakkad grows special varieties)"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 3,
    // "notes": [
      // "Silent Valley is one of the last pristine tropical rainforests in India — restricted access keeps it genuinely wild.",
      // "Palakkad is a gateway city that most Kerala tourists skip entirely; the fort and Nelliyampathy hills alone justify a night here.",
      // "The Palakkad gap (the only break in the Western Ghats) gives this district a different climate — hotter and drier than the rest of Kerala."
    // ]
  },
  "gwalior": {
    "city": "Gwalior",
    "lat": 26.22983,
    "lng": 78.17337,
    "state": "madhya_pradesh",
    "region": "central",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "in_transit"
    ],
    "must_visit": [
      "Gwalior Fort",
      "Man Singh Palace",
      "Jai Vilas Palace",
      "Tansen's Tomb",
      "Sas Bahu Temples",
      "Gopachal Parvat (Jain rock sculptures)"
    ],
    "must_try": [
      "Gwalior Peda",
      "Dal Tadka at local dhabas",
      "Evening sound and light show at the fort",
      "Tansen Music Festival (if in season)",
      "Cycling through the old city"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "Gwalior Fort is one of the most underrated forts in India — it sits on a sheer rock plateau above the city and the Man Singh Palace interior is extraordinary.",
      // "The Jain rock sculptures at Gopachal — massive figures carved directly into the cliff face — are completely overlooked by tourists and utterly haunting.",
      // "Jai Vilas Palace has one of the strangest Durbar halls in India: two of the world's largest chandeliers, and a silver toy train that served drinks at dinner."
    // ]
  },
  "nagpur": {
    "city": "Nagpur",
    "lat": 21.14631,
    "lng": 79.08491,
    "state": "maharashtra",
    "region": "central",
    "category": [
      "metro",
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "Deekshabhoomi",
      "Dragon Palace Buddhist Temple",
      "Ambazari Lake",
      "Sitabuldi Fort",
      "Zero Mile Stone",
      "Tadoba Tiger Reserve (day trip)"
    ],
    "must_try": [
      "Saoji mutton curry",
      "Tarri Poha",
      "Nagpuri oranges",
      "Sabudana khichdi",
      "Tadoba safari",
      "Ambedkar Memorial visit"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 3,
    // "notes": [
      // "Nagpur is India's geographic centre and it has a certain unhurried self-confidence that bigger cities don't — Deekshabhoomi alone is worth the stop.",
      // "Saoji food is Nagpur's best-kept secret — the mutton is slow-cooked in a masala blend unique to the Saoji community and it is incendiary in the best way.",
      // "Tadoba is consistently one of the best tiger sighting destinations in India — if you're in Nagpur, blocking a morning for a safari is non-negotiable."
    // ]
  },
  "gokarna": {
    "city": "Gokarna",
    "lat": 14.55,
    "lng": 74.31667,
    "state": "karnataka",
    "region": "southern",
    "category": [
      "coastal",
      "pilgrimage",
      "backpacker"
    ],
    "must_visit": [
      "Om Beach",
      "Mahabaleshwar Temple (Atmalinga)",
      "Half Moon Beach",
      "Paradise Beach",
      "Kudle Beach",
      "Mirjan Fort"
    ],
    "must_try": [
      "beach trek from Om Beach to Paradise Beach",
      "sunset on Half Moon Beach",
      "Mahabaleshwar Temple lingam darshan",
      "fresh fish thali at beach shacks",
      "Om Beach hammock and book afternoon",
      "cliff jumping at Half Moon Beach",
      "Gokarna full-moon beach party"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller"
    ],
    "mode_of_travel": [
      "auto",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Gokarna is the rare place where a major pilgrimage temple and a backpacker beach scene exist in complete harmony.",
      // "The Paradise Beach trek is 45 minutes of rocky coast scrambling — worth every bruise for the isolation at the end.",
      // "Unlike Goa, Gokarna still feels genuinely undiscovered; locals and pilgrims outnumber tourists most of the year."
    // ]
  },
  "bhuj": {
    "city": "Bhuj",
    "lat": 23.25397,
    "lng": 69.66928,
    "state": "gujarat",
    "region": "western",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "desert"
    ],
    "must_visit": [
      "Bhuj Swaminarayan Temple",
      "Aina Mahal",
      "Prag Mahal",
      "Hamirsar Lake",
      "Bhuj Rao Lakha Palace",
      "Chhatardi"
    ],
    "must_try": [
      "Kutchi Dabeli",
      "Kutchi Bajra Rotla",
      "Camel safari at Rann",
      "White Rann sunset",
      "Kutchi handicraft shopping",
      "Kutchi embroidery"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "ola/uber",
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Base city for the Rann of Kutch — plan your visit around Rann Utsav (Nov-Feb) for the full desert festival experience.",
      // "The 2001 earthquake devastated the city but the rebuilt temples and restored havelis are remarkable in their own right.",
      // "Bhujodi village 8km away is the epicentre of Kutchi handicrafts — better prices and selection than the city bazaars."
    // ]
  },
  "kutchh": {
    "city": "Kutchh",
    "lat": 23.25397,
    "lng": 69.66928,
    "state": "gujarat",
    "region": "western",
    "category": [
      "monuments_museums",
      "desert"
    ],
    "must_visit": [
      "White Rann",
      "Kala Dungar (Black Hill)",
      "Flamingo City",
      "Dholavira (Indus Valley site)",
      "Mandvi Beach",
      "Wild Ass Sanctuary"
    ],
    "must_try": [
      "Rann Utsav tent stay",
      "Camel cart ride on white salt desert",
      "Flamingo watching at dawn",
      "Star gazing on the Rann",
      "Kutchi thali"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "bike_rentals"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 5,
    // "notes": [
      // "The White Rann at full moon is one of the most surreal landscapes in India — the salt flats reflect the moon light like a giant mirror.",
      // "Dholavira is one of the largest Indus Valley sites in India and criminally undervisited — a 4500-year-old city hiding in plain sight.",
      // "Entry to the Rann requires a permit — book online before you arrive, especially during Utsav season."
    // ]
  },
  "bhopal": {
    "city": "Bhopal",
    "lat": 23.25469,
    "lng": 77.40289,
    "state": "madhya_pradesh",
    "region": "central",
    "category": [
      "metro",
      "monuments_museums",
      "in_transit",
      "base_camp"
    ],
    "must_visit": [
      "Bhimbetka Rock Shelters",
      "Sanchi Stupa",
      "Upper Lake (Bada Talab)",
      "Van Vihar National Park",
      "Taj-ul-Masajid",
      "Bharat Bhavan"
    ],
    "must_try": [
      "Bhopal biryani",
      "Seekh kabab at Chatori Gali",
      "Shahi korma",
      "Boat ride on Upper Lake",
      "Bhimbetka half-day trip",
      "Old city street food walk"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "ola/uber",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Bhimbetka's 30,000-year-old rock paintings are genuinely among the most important art sites on earth and you can walk right up to them — the lack of crowds feels almost irresponsible.",
      // "Bhopal's dual personality — a modern state capital sitting beside one of India's oldest cities — gives it a layered quality that rewards slow exploration.",
      // "The Nawabi food culture here is seriously underrated; the Old City restaurants serve Mughal-era preparations that have been quietly perfected over generations."
    // ]
  },
  "udupi": {
    "city": "Udupi",
    "lat": 13.33467,
    "lng": 74.74617,
    "state": "karnataka",
    "region": "southern",
    "category": [
      "pilgrimage",
      "coastal"
    ],
    "must_visit": [
      "Sri Krishna Temple",
      "Malpe Beach",
      "St. Mary's Island",
      "Udupi Manipal",
      "Anantheshwara Temple",
      "Kaup Lighthouse & Beach"
    ],
    "must_try": [
      "Udupi meals at the original Woodlands or MTR-style mess",
      "Udupi sambar and coconut chutney (the originals)",
      "St. Mary's Island boat trip (columnar basalt formations)",
      "Krishna Temple paryaya festival (every 2 years)",
      "masala dosa at source",
      "Kaup beach lighthouse sunset",
      "goli baje (unique Udupi fritter)"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "auto",
      "city_bus",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "Udupi is where the masala dosa and sambar were invented — eating them here in a traditional mess is a pilgrimage of its own kind.",
      // "St. Mary's Island's hexagonal basalt columns rising from the sea look like they belong in Iceland, not Karnataka.",
      // "The Krishna Temple has a fascinating tradition where you view the deity only through a hole in a window — unique to Udupi."
    // ]
  },
  "manglore": {
    "city": "Manglore",
    "lat": 12.91723,
    "lng": 74.85603,
    "state": "karnataka",
    "region": "southern",
    "category": [
      "coastal",
      "monuments_museums",
      "metro",
      "in_transit"
    ],
    "must_visit": [
      "Kadri Manjunath Temple",
      "Panambur Beach",
      "Sultan Battery",
      "St. Aloysius College Chapel",
      "Kudroli Gokarnath Temple",
      "Mangaladevi Temple"
    ],
    "must_try": [
      "Mangalorean fish curry with neer dosa",
      "kori rotti (chicken with crispy rice wafers)",
      "bangude (mackerel) fry at local restaurants",
      "St. Aloysius Chapel frescoes visit",
      "Panambur beach sunset",
      "Mangalorean buns (sweet fried bread)",
      "port area seafood market in the morning"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "city_bus",
      "auto",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Mangalore's cuisine is criminally underrated nationally — the coconut-based seafood dishes here are distinct from both Goan and Keralite food.",
      // "St. Aloysius Chapel has Italian-style ceiling frescoes that look completely out of place in coastal Karnataka — that's exactly what makes them worth seeing.",
      // "Mangalore is a transit hub more than a destination but deserves at least one proper day for the food alone."
    // ]
  },
  "kannur": {
    "city": "Kannur",
    "lat": 11.86752,
    "lng": 75.35763,
    "state": "kerala",
    "region": "southern",
    "category": [
      "coastal",
      "forts_palaces",
      "pilgrimage"
    ],
    "must_visit": [
      "Theyyam performances (Oct–May)",
      "St. Angelo Fort",
      "Parassinikadavu Snake Park",
      "Payyambalam Beach",
      "Muzhappilangad Drive-in Beach",
      "Ezhimala Naval Academy (viewpoint)"
    ],
    "must_try": [
      "Theyyam ritual performance at village temples",
      "Muzhappilangad Drive-in Beach drive",
      "Kannur fish curry with tapioca",
      "St. Angelo Fort sunset",
      "Dharmadam Island boat trip",
      "locally woven handloom fabrics (Kannur is the weaving capital)"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "city_bus",
      "auto",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Theyyam is the reason to come to Kannur — a ritual art form where performers become divine — completely unmediated by tourism.",
      // "Muzhappilangad is India's only drive-in beach; watching the sunset from the car with waves crashing is oddly cinematic.",
      // "Kannur's handloom cooperatives produce some of the finest cotton fabrics in India; buying direct here is excellent value."
    // ]
  },
  "kozhikode": {
    "city": "Kozhikode",
    "lat": 11.24802,
    "lng": 75.7804,
    "state": "kerala",
    "region": "southern",
    "category": [
      "monuments_museums",
      "coastal",
      "backpacker"
    ],
    "must_visit": [
      "Kozhikode Beach",
      "Kappad Beach (Vasco da Gama landing site)",
      "Thikkoti Lighthouse",
      "Mishkal Mosque (Kuttichira)",
      "Gundert Bungalow",
      "Sweet Meat Street (SM Street)"
    ],
    "must_try": [
      "Kozhikode biryani (Malabar biryani)",
      "Kozhikode halwa from SM Street",
      "pathiri with fish molee",
      "thalassery biryani comparison",
      "Kozhikode beach sunset with banana fritters",
      "kallumakkaya (mussels) ularthiyathu",
      "heritage walk through Kuttichira"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "city_bus",
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "Kozhikode (Calicut) was the gateway of the spice trade — Vasco da Gama landed here, and the city still carries that cosmopolitan mercantile atmosphere.",
      // "The Malabar biryani here uses thin-grained rice and is gentler than Hyderabadi style — two different religions of biryani.",
      // "SM Street's halwa shops have been operating for generations; the black halwa is unique to Kozhikode and worth bringing home."
    // ]
  },
  "dalhouisie": {
    "city": "Dalhouisie",
    "lat": 32.55219,
    "lng": 75.94663,
    "state": "himachal_pradesh",
    "region": "northern",
    "category": [
      "hill_station"
    ],
    "must_visit": [
      "Dainkund Peak",
      "Khajjiar (Mini Switzerland)",
      "Subhash Baoli",
      "St. Francis Church",
      "Bakrota Hills walk"
    ],
    "must_try": [
      "Siddu at local dhabas",
      "Khajjiar meadow walk and zip-line",
      "Chukh (Himachali pickle) shopping",
      "Sunset from Dainkund Peak",
      "Colonial-era bakery pastries in the mall area"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller"
    ],
    "mode_of_travel": [
      "ola/uber"
    ],
    "ideal_season": "summer",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Khajjiar in the monsoon when the mist rolls in over the meadow feels genuinely Alpine.",
      // "Dalhousie has kept its colonial character better than most hill stations — the churches and old bungalows are lovely.",
      // "It is significantly less crowded than Shimla or Manali, which in itself is a reason to come."
    // ]
  },
  "dharamshala-mcleond-gang": {
    "city": "Dharamshala & Mcleond Gang",
    "lat": 32.22006,
    "lng": 76.32013,
    "state": "himachal_pradesh",
    "region": "northern",
    "category": [
      "hill_station",
      "pilgrimage",
      "backpacker"
    ],
    "must_visit": [
      "Namgyal Monastery & Tsuglagkhang Complex",
      "Bhagsu Nag Waterfall",
      "Triund Trek",
      "Dal Lake Dharamshala",
      "Tibet Museum"
    ],
    "must_try": [
      "Tibetan thukpa and momos at McLeod Ganj cafes",
      "Butter tea at a Tibetan monastery",
      "Triund overnight trek",
      "Dharamshala Cricket Stadium tour",
      "Prayer wheel circuit around the main temple",
      "Israeli cafe scene in McLeod Ganj"
    ],
    "accommodation": [
      "zostel",
      "gostops",
      "the_hosteller",
      "madpackers"
    ],
    "mode_of_travel": [
      "ola/uber",
      "bike_rental"
    ],
    "ideal_season": "summer",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "McLeod Ganj has a unique energy — Tibetan monks, Israeli backpackers, Indian pilgrims, and serious trekkers all coexist.",
      // "Triund trek is the most accessible overnight high-altitude experience in India — do it at least once.",
      // "The Tsuglagkhang complex when the Dalai Lama is in residence has an electric, rare atmosphere."
    // ]
  },
  "bir": {
    "city": "Bir",
    "lat": 32.0409,
    "lng": 76.7187,
    "state": "himachal_pradesh",
    "region": "northern",
    "category": [
      "backpacker"
    ],
    "must_visit": [
      "Bir Billing paragliding launch site",
      "Sherab Ling Monastery",
      "Bir Tea Garden",
      "Tibetan Colony Market",
      "Billing viewpoint"
    ],
    "must_try": [
      "Paragliding from Billing (world-class launch site)",
      "Tibetan momos at the colony",
      "Organic tea from Bir Tea Garden",
      "Mountain biking on forest trails",
      "Camping at Billing overnight",
      "Sunrise view of Dhauladhar range"
    ],
    "accommodation": [
      "gostops",
      "the_hosteller",
      "moustache",
      "madpackers"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Bir Billing is the paragliding capital of Asia — if you are going to do paragliding in India, do it here.",
      // "The Tibetan refugee colony has an authentic calm that McLeod Ganj has somewhat lost to tourism.",
      // "Bir is still under the radar enough that you can find genuinely quiet guesthouses — book ahead in October."
    // ]
  },
  "wayanad-vyithri": {
    "city": "Wayanad(Vyithri)",
    "lat": 11.6854,
    "lng": 76.132,
    "state": "kerala",
    "region": "southern",
    "category": [
      "hill_station",
      "monuments_museums"
    ],
    "must_visit": [
      "Edakkal Caves",
      "Chembra Peak",
      "Banasura Sagar Dam",
      "Pookode Lake",
      "Soochipara Waterfalls",
      "Thirunelli Temple"
    ],
    "must_try": [
      "trek to Chembra Peak (heart-shaped lake at top)",
      "bamboo rafting at Pookode Lake",
      "Wayanad tribal village visit",
      "bamboo rice puttu",
      "jungle night safari",
      "coffee and pepper estate walk",
      "jackfruit halwa"
    ],
    "accommodation": [
      "zostel",
      "the_hosteller"
    ],
    "mode_of_travel": [
      "bike_rental"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "3-4 days",
    "rating": 5,
    // "notes": [
      // "Chembra Peak trek is one of the best day treks in South India — the heart-shaped lake at the top is genuinely one of a kind.",
      // "Wayanad is misty, green, and tribal in character — the coffee estates here feel more authentic than Coorg.",
      // "Roads to Wayanad through the Ghat sections are genuinely beautiful — don't rush this drive."
    // ]
  },
  "mahabalipuram": {
    "city": "Mahabalipuram",
    "lat": 12.6267,
    "lng": 80.1927,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "monuments_museums",
      "coastal"
    ],
    "must_visit": [
      "Shore Temple",
      "Pancha Rathas (Five Rathas)",
      "Arjuna's Penance (rock relief)",
      "Mahishasura Mardini Cave",
      "Krishna's Butter Ball",
      "Tiger Cave"
    ],
    "must_try": [
      "Shore Temple sunrise",
      "Arjuna's Penance sculpture close-up study",
      "rock-climbing on Krishna's Butter Ball",
      "local fishermen's beach morning walk",
      "Mahabalipuram stone carving workshop visit",
      "fresh lobster at a beach shack",
      "sunset from the Shore Temple beach"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "bike_rental",
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "Arjuna's Penance is one of the largest bas-relief sculptures in the world — stand back from it and the scale genuinely impresses.",
      // "Mahabalipuram is 55km from Chennai and one of India's great day trips; staying overnight lets you have the Shore Temple to yourself at dawn.",
      // "The stone carving tradition here is still alive — the workshops along the main road are worth poking your head into."
    // ]
  },
  "thanjavur": {
    "city": "Thanjavur",
    "lat": 10.78523,
    "lng": 79.13909,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "pilgrimage"
    ],
    "must_visit": [
      "Brihadeeswarar Temple (Big Temple)",
      "Thanjavur Royal Palace",
      "Saraswathi Mahal Library",
      "Thanjavur Art Gallery",
      "Sivaganga Garden",
      "Airavateeswara Temple (Darasuram)"
    ],
    "must_try": [
      "Brihadeeswarar Temple at sunrise",
      "Chola bronze sculpture close-up study",
      "Thanjavur painting workshop (tanjore art)",
      "Saraswathi Mahal rare manuscript viewing",
      "Thanjavur special rice and sambar meal",
      "Airavateeswara Temple (a UNESCO site often skipped)",
      "Carnatic classical music performance"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "city_bus",
      "bike_rental"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 5,
    // "notes": [
      // "The Brihadeeswarar Temple is a 1000-year-old Chola masterpiece — the vimana (tower) casts no shadow at noon, a feat of ancient engineering.",
      // "Thanjavur is the Chola capital and the birthplace of Bharatanatyam and Carnatic music — the cultural density here is extraordinary.",
      // "The Saraswathi Mahal Library has manuscripts in 49 languages including rare Sanskrit and Tamil palm-leaf manuscripts; genuinely moving to see."
    // ]
  },
  "tiruchirappalli": {
    "city": "Tiruchirappalli",
    "lat": 10.8155,
    "lng": 78.69651,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "forts_palaces",
      "monuments_museums",
      "pilgrimage",
      "in_transit"
    ],
    "must_visit": [
      "Sri Ranganathaswamy Temple (Srirangam)",
      "Rock Fort Temple (Uchi Pillaiyar)",
      "Jambukeswarar Temple (Thiruvanaikaval)",
      "Lourdes Church",
      "Kallanai Dam (Grand Anicut)",
      "Samayapuram Mariamman Temple"
    ],
    "must_try": [
      "Rock Fort Temple climb (344 steps to the top)",
      "Srirangam temple complex corridor walk (largest functioning temple in world by area)",
      "Jambukeswarar lingam submerged in water (unique Shiva shrine)",
      "Trichy jigarthanda (different from Madurai's)",
      "local biryani near Chathram bus stand",
      "banana leaf meal at a hotel near Srirangam"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "city_bus",
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "2-3 days",
    "rating": 4,
    // "notes": [
      // "Srirangam is the largest functioning temple in the world by area — exploring all its concentric prakarams takes a full day.",
      // "Rock Fort at sunset with the Kaveri river below and Srirangam's gopurams in the distance is one of Tamil Nadu's great views.",
      // "Trichy is often overlooked on the Madurai-Chennai route but has some of the most important Dravidian temple architecture in existence."
    // ]
  },
  "kanyakumari": {
    "city": "Kanyakumari",
    "lat": 8.09008,
    "lng": 77.53841,
    "state": "tamil_nadu",
    "region": "southern",
    "category": [
      "pilgrimage",
      "coastal",
      "forts_palaces"
    ],
    "must_visit": [
      "Vivekananda Rock Memorial",
      "Thiruvalluvar Statue",
      "Kanyakumari Temple (Devi Kanya Kumari)",
      "Sunset & Sunrise Point",
      "Gandhi Memorial Mandapam",
      "Padmanabhapuram Palace"
    ],
    "must_try": [
      "sunrise and sunset from the same spot (three seas meet here)",
      "boat ride to Vivekananda Rock and Thiruvalluvar Statue",
      "Kanyakumari temple darshan at high tide",
      "Padmanabhapuram Palace Keralan wood architecture tour",
      "local seashell and coral handicraft shopping",
      "fresh coconut water at land's end",
      "kothu parotta with chicken gravy"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "auto",
      "city_bus"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "Kanyakumari is India's land's end — three oceans meet here and on a clear day you can watch both sunset and moonrise over the water simultaneously.",
      // "The Vivekananda Rock visit is genuinely moving — the meditation hall feels like the edge of the world.",
      // "It's worth staying one night just for the dawn — the tri-coloured waters (Arabian Sea, Bay of Bengal, Indian Ocean) at sunrise are unmistakable."
    // ]
  },
  "mayapur": {
    "city": "Mayapur",
    "lat": 23.4167,
    "lng": 88.4,
    "state": "west_bengal",
    "region": "eastern",
    "category": [
      "pilgrimage",
      "backpacker"
    ],
    "must_visit": [
      "ISKCON Chandrodaya Mandir",
      "Samadhi of Srila Prabhupada",
      "Yoga Pitha (birthplace of Chaitanya Mahaprabhu)",
      "Ganga-Jalangi confluence",
      "Chandrodaya Mandir Tower of Vedic Knowledge (under construction)"
    ],
    "must_try": [
      "Full prasadam meal at ISKCON dining hall",
      "Morning kirtan at 4:30am (Mangala Aarti)",
      "Boat ride on the Ganges",
      "Vaishnav khichdi prasad",
      "Walk the parikrama path around Mayapur dhama"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 4,
    // "notes": [
      // "Mangala Aarti at 4:30am inside the main temple is one of the most electrically devotional experiences in India.",
      // "The prasadam here is extraordinary — and free or nearly free for pilgrims.",
      // "Even as a non-devotee the scale of the ISKCON vision here is breathtaking — the temple complex being built is the size of a small city."
    // ]
  },
  "mirik": {
    "city": "Mirik",
    "lat": 26.88817,
    "lng": 88.19007,
    "state": "west_bengal",
    "region": "eastern",
    "category": [
      "hill_station",
      "backpacker"
    ],
    "must_visit": [
      "Mirik Lake (Sumendu Lake)",
      "Bokar Gompa Monastery",
      "Orange Orchards",
      "Thurbo Tea Estate",
      "Ramitey Viewpoint"
    ],
    "must_try": [
      "Paddle boat on Mirik lake",
      "Tea tasting at Thurbo estate",
      "Momos at lakeside stalls",
      "Drive through orange orchards in season (Oct-Nov)",
      "Dawn walk along the lake promenade"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [],
    "ideal_season": "post_monsoon",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Mirik is Darjeeling without the crowds and with a gentler pace — a best-kept-ish secret.",
      // "The lake is serene in the early morning before the day-trippers arrive around 10am.",
      // "Perfect as a one-night detour between Siliguri and Darjeeling."
    // ]
  },
  "kurseong": {
    "city": "Kurseong",
    "lat": 27.03333,
    "lng": 88.26667,
    "state": "west_bengal",
    "region": "eastern",
    "category": [
      "hill_station",
      "monuments_museums",
      "backpacker"
    ],
    "must_visit": [
      "Cochrane Place (heritage property)",
      "Eagle's Craig Viewpoint",
      "Dow Hill Forest",
      "Netaji Subhas Chandra Bose Museum",
      "Ambotia Tea Estate"
    ],
    "must_try": [
      "Toy train journey through Kurseong",
      "Tea at Ambotia estate with valley views",
      "Bakery breakfast at local hillside cafes",
      "Walk the Dow Hill Forest trail",
      "Sunset from Eagle's Craig"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [],
    "ideal_season": "post_monsoon",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Kurseong is the quietest of the hill towns on the Darjeeling loop — and that's its greatest asset.",
      // "Dow Hill has a reputation for being haunted; whether or not you believe it, the forest is legitimately eerie and beautiful.",
      // "The Toy Train pass through here is the most scenic stretch — windows down, tea in hand."
    // ]
  },
  "ghoom": {
    "city": "Ghoom",
    "lat": 27.03333,
    "lng": 88.26667,
    "state": "west_bengal",
    "region": "eastern",
    "category": [
      "hill_station",
      "monuments_museums",
      "pilgrimage"
    ],
    "must_visit": [
      "Ghoom Monastery (Yiga Choeling)",
      "Batasia Loop",
      "Ghoom Railway Station (highest on DHR)",
      "Tiger Hill (nearby)",
      "Observatory Hill"
    ],
    "must_try": [
      "Early morning Toy Train ride through Ghoom",
      "Butter tea inside the monastery",
      "Sunrise from Tiger Hill with Ghoom as base",
      "Monk chanting ceremony at Ghoom Monastery",
      "Local thukpa in the tiny market"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [],
    "ideal_season": "post_monsoon",
    "ideal_stay": "1 day",
    "rating": 3,
    // "notes": [
      // "Ghoom Monastery holds a 15-foot Maitreya Buddha statue that's been quietly sitting there since 1875 — find it at the right light and it stops you cold.",
      // "The highest railway station in India is here and most people blow through it — get out and stand on the platform for five minutes.",
      // "It's technically a Darjeeling extension but has its own distinct, slower quality."
    // ]
  },
  "kalimpong": {
    "city": "Kalimpong",
    "lat": 27.07059,
    "lng": 88.47529,
    "state": "west_bengal",
    "region": "eastern",
    "category": [
      "hill_station",
      "backpacker"
    ],
    "must_visit": [
      "Zang Dhok Palri Phodang Monastery",
      "Tharpa Choling Monastery",
      "Deolo Hill",
      "Kalimpong Science City",
      "Durpin Dara Hill",
      "Flower Nurseries"
    ],
    "must_try": [
      "Nak-thuk (yak milk soup)",
      "Gyathuk (Tibetan noodle soup)",
      "Shop for orchids and cacti at nurseries",
      "Drive-in views of Teesta Valley",
      "Visit the Sunday haat for local produce",
      "Eat at a Tibetan refugee family's homestay"
    ],
    "accommodation": [
      "homestays"
    ],
    "mode_of_travel": [
      "private_taxi"
    ],
    "ideal_season": "post_monsoon",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Kalimpong has a strong Tibetan and Lepcha heritage that gives it a different flavor from Darjeeling — less touristy, more layered.",
      // "The flower nurseries here supply much of India's orchid trade — a completely unexpected thing to discover in a hill town.",
      // "The views of Teesta River from Deolo Hill are some of the best in the eastern Himalayas."
    // ]
  },
  "patna": {
    "city": "Patna",
    "lat": 25.59408,
    "lng": 85.13563,
    "state": "bihar",
    "region": "eastern",
    "category": [
      "metro",
      "monuments_museums",
      "pilgrimage",
      "base_camp"
    ],
    "must_visit": [
      "Golghar",
      "Patna Museum",
      "Agam Kuan",
      "Mahavir Mandir",
      "Patna Sahib Gurudwara",
      "Gandhi Maidan"
    ],
    "must_try": [
      "Litti chokha (authentic, not Delhi version)",
      "Sattu paratha",
      "Tilkut from Gaya vendors",
      "Patna Market chaat",
      "Bihari thali at a local restaurant",
      "Boat ride on the Ganges at sunset"
    ],
    "accommodation": [
      "budget_hotels"
    ],
    "mode_of_travel": [
      "auto",
      "city_bus",
      "ola/uber"
    ],
    "ideal_season": "winter",
    "ideal_stay": "1-2 days",
    "rating": 3,
    // "notes": [
      // "Patna is often dismissed but it has layers — Mauryan empire roots, Sikh significance, Buddhist proximity.",
      // "Litti chokha here is the reference version — everything else you've eaten is an approximation.",
      // "The Ganga ghats at Patna are less famous than Varanasi's but give you more space to actually be present."
    // ]
  }
};

const ACCOMMODATION_META = {
  "bloom": {
    "label": "Bloom",
    "slug": "bloom"
  },
  "zostel": {
    "label": "Zostel",
    "slug": "zostel"
  },
  "gostops": {
    "label": "goSTOPS",
    "slug": "gostops"
  },
  "the_hosteller": {
    "label": "The Hosteller",
    "slug": "the-hosteller"
  },
  "moustache": {
    "label": "Moustache",
    "slug": "moustache"
  },
  "madpackers": {
    "label": "Madpackers",
    "slug": "madpackers"
  },
  "homestays": {
    "label": "Homestays",
    "slug": "homestays"
  },
  "budget_hotels": {
    "label": "Hotels",
    "slug": "hotels"
  }
};

export const ACCOMMODATIONS = Object.fromEntries(
  Object.entries(ACCOMMODATION_META)
    .map(([code, meta]) => [
      code,
      {
        ...meta,
        cities: Object.entries(CITIES)
          .filter(([, city]) => city.accommodation?.includes(code))
          .map(([cityCode]) => cityCode)
          .sort((a, b) => CITIES[a].city.localeCompare(CITIES[b].city))
      }
    ])
    .filter(([, group]) => group.cities.length > 0)
);

export const CATEGORIES = {
  "backpacker": {
    "label": "Backpacker",
    "slug": "backpacker",
    "cities": [
      "allepey",
      "bir",
      "bodh-gaya",
      "dawki",
      "dharamshala-mcleond-gang",
      "digha",
      "dimapur",
      "goa",
      "gokarna",
      "guwahati",
      "hampi",
      "itanagar",
      "kalimpong",
      "kharagpur",
      "kochi",
      "kodaikannal",
      "kohima",
      "kolkata",
      "kozhikode",
      "kurseong",
      "leh",
      "manali",
      "mayapur",
      "mirik",
      "pondicherry",
      "poombarai",
      "shillong",
      "silchar",
      "siliguri",
      "srinagar",
      "tinsukia",
      "vagamon",
      "varanasi",
      "varkala"
    ]
  },
  "coastal": {
    "label": "Coastal",
    "slug": "coastal",
    "cities": [
      "allepey",
      "chennai",
      "digha",
      "dwarka",
      "goa",
      "gokarna",
      "kannur",
      "kanyakumari",
      "kochi",
      "kozhikode",
      "mahabalipuram",
      "manglore",
      "mumbai",
      "pondicherry",
      "puri-konark",
      "rameshwaram",
      "somnath",
      "trivandrum",
      "udupi",
      "varkala",
      "vishakhapatnam"
    ]
  },
  "desert": {
    "label": "Desert",
    "slug": "desert",
    "cities": [
      "bhuj",
      "bikaner",
      "jaisalmer",
      "kutchh",
      "sam"
    ]
  },
  "food": {
    "label": "Food",
    "slug": "food",
    "cities": [
      "ahmedabad-gandhinagar",
      "amritsar",
      "bengaluru",
      "delhi",
      "hyderabad",
      "indore",
      "jaipur",
      "kochi",
      "kolkata",
      "kozhikode",
      "lukhnow",
      "madurai",
      "mumbai",
      "mudurai",
      "nashik",
      "pune",
      "surat",
      "varanasi"
    ]
  },
  "forts_palaces": {
    "label": "Forts and Palaces",
    "slug": "forts-and-palaces",
    "cities": [
      "agra",
      "amritsar",
      "bhuj",
      "bikaner",
      "chennai",
      "chittorgarh",
      "cuttak",
      "delhi",
      "goa",
      "gwalior",
      "hyderabad",
      "itanagar",
      "jaipur",
      "jaisalmer",
      "jammu",
      "jhansi",
      "jodhpur",
      "kannur",
      "kanyakumari",
      "kochi",
      "kumbhalgarh",
      "leh",
      "madurai",
      "mudurai",
      "mysore",
      "orchha",
      "palakkad",
      "prayagraj",
      "pune",
      "sambhajinagar",
      "shivamogga",
      "thanjavur",
      "tiruchirappalli",
      "udaipur",
      "vadodara"
    ]
  },
  "hill_station": {
    "label": "Mountains",
    "slug": "mountains",
    "cities": [
      "araku",
      "chikkmanglore",
      "coorg",
      "dalhouisie",
      "darjeeling",
      "dharamshala-mcleond-gang",
      "gangtok",
      "ghoom",
      "gulmarg",
      "kalimpong",
      "kodaikannal",
      "kullu",
      "kurseong",
      "manali",
      "mirik",
      "munnar",
      "ooty",
      "pelling",
      "poombarai",
      "shillong",
      "sonmarg",
      "srinagar",
      "tawang",
      "theni",
      "vagamon",
      "wayanad",
      "wayanad-vyithri"
    ]
  },
  "metro": {
    "label": "Metro",
    "slug": "metro",
    "cities": [
      "ahmedabad-gandhinagar",
      "bengaluru",
      "bhopal",
      "chennai",
      "coimbatore",
      "delhi",
      "gurugram",
      "guwahati",
      "hyderabad",
      "indore",
      "jaipur",
      "kochi",
      "kolkata",
      "manglore",
      "mumbai",
      "nagpur",
      "noida",
      "patna",
      "pune",
      "rajkot",
      "surat",
      "trivandrum",
      "vishakhapatnam"
    ]
  },
  "in_transit": {
    "label": "In Transit",
    "slug": "in-transit",
    "cities": [
      "bhopal",
      "coimbatore",
      "cuttak",
      "dimapur",
      "gwalior",
      "indore",
      "jalandhar",
      "jammu",
      "jhansi",
      "kharagpur",
      "ludhiana",
      "manglore",
      "nagpur",
      "palakkad",
      "pathankot",
      "rajkot",
      "silchar",
      "siliguri",
      "surat",
      "theni",
      "tiruchirappalli",
      "vadodara"
    ]
  },
  "monuments_museums": {
    "label": "Monuments and Museums",
    "slug": "monuments-and-museums",
    "cities": [
      "agra",
      "ahmedabad-gandhinagar",
      "amritsar",
      "araku",
      "bhopal",
      "bhuj",
      "bikaner",
      "bodh-gaya",
      "chandigarh",
      "chennai",
      "chittorgarh",
      "cuttak",
      "darjeeling",
      "delhi",
      "dimapur",
      "gangtok",
      "ghoom",
      "goa",
      "gwalior",
      "hampi",
      "hyderabad",
      "itanagar",
      "jaipur",
      "jaisalmer",
      "jammu",
      "jhansi",
      "jodhpur",
      "kanchipuram",
      "kargil",
      "kevadiya",
      "khajurao",
      "kochi",
      "kohima",
      "kolkata",
      "kozhikode",
      "kurseong",
      "kutchh",
      "leh",
      "lukhnow",
      "madurai",
      "mahabalipuram",
      "manglore",
      "mudurai",
      "mysore",
      "orchha",
      "patna",
      "pondicherry",
      "prayagraj",
      "pune",
      "puri-konark",
      "rameshwaram",
      "sambhajinagar",
      "silchar",
      "srinagar",
      "thanjavur",
      "thrissur",
      "tiruchirappalli",
      "varanasi",
      "vishakhapatnam",
      "wayanad",
      "wayanad-vyithri"
    ]
  },
  "nature": {
    "label": "Scenic",
    "slug": "scenic",
    "cities": [
      "allepey",
      "araku",
      "baltal",
      "bhuj",
      "bir",
      "chikkmanglore",
      "coorg",
      "dawki",
      "digha",
      "gangtok",
      "gokarna",
      "gulmarg",
      "itanagar",
      "kannur",
      "kanyakumari",
      "kodaikannal",
      "kohima",
      "kullu",
      "kutchh",
      "leh",
      "manali",
      "mirik",
      "munnar",
      "ooty",
      "palakkad",
      "pelling",
      "poombarai",
      "shillong",
      "shivamogga",
      "silchar",
      "siliguri",
      "tawang",
      "theni",
      "thrissur",
      "tinsukia",
      "udupi",
      "vagamon",
      "varkala",
      "wayanad",
      "wayanad-vyithri"
    ]
  },
  "pilgrimage": {
    "label": "Temple Tour",
    "slug": "temple-tour",
    "cities": [
      "ajmer-pushkar",
      "amritsar",
      "ayodhya",
      "baltal",
      "bodh-gaya",
      "cuttak",
      "delhi",
      "deoghar",
      "dharamshala-mcleond-gang",
      "dwarka",
      "ghoom",
      "gokarna",
      "guwahati",
      "jammu",
      "kanchipuram",
      "kannur",
      "kanyakumari",
      "katra",
      "leh",
      "madurai",
      "mathura",
      "mayapur",
      "mudurai",
      "mysore",
      "nashik",
      "omkareshwar",
      "patna",
      "pelling",
      "prayagraj",
      "puri-konark",
      "rameshwaram",
      "shirdi",
      "somnath",
      "srisailam",
      "tawang",
      "thanjavur",
      "thrissur",
      "tiruchirappalli",
      "tirupati",
      "trivandrum",
      "udupi",
      "varanasi",
      "varkala"
    ]
  },
  "border_town": {
    "label": "Border Town",
    "slug": "border-town",
    "cities": [
      "amritsar",
      "dawki",
      "dimapur",
      "kargil",
      "pathankot",
      "tawang"
    ]
  },
  "base_camp": {
    "label": "Base Camp",
    "slug": "base-camp",
    "cities": [
      "bengaluru",
      "bhopal",
      "chandigarh",
      "chennai",
      "coimbatore",
      "delhi",
      "dimapur",
      "gurugram",
      "guwahati",
      "hyderabad",
      "jammu",
      "kharagpur",
      "kochi",
      "kolkata",
      "ludhiana",
      "mumbai",
      "nagpur",
      "noida",
      "pathankot",
      "patna",
      "pune",
      "silchar",
      "siliguri",
      "surat",
      "vadodara"
    ]
  }
};

export const REGIONS = {
  "central": {
    "label": "Central",
    "slug": "central",
    "cities": [
      "bhopal",
      "gwalior",
      "indore",
      "khajurao",
      "omkareshwar",
      "orchha"
    ]
  },
  "eastern": {
    "label": "Eastern",
    "slug": "eastern",
    "cities": [
      "bodh-gaya",
      "cuttak",
      "darjeeling",
      "deoghar",
      "digha",
      "ghoom",
      "kalimpong",
      "kharagpur",
      "kolkata",
      "kurseong",
      "mayapur",
      "mirik",
      "patna",
      "puri-konark",
      "siliguri"
    ]
  },
  "northeastern": {
    "label": "Northeastern",
    "slug": "northeastern",
    "cities": [
      "dawki",
      "dimapur",
      "gangtok",
      "guwahati",
      "itanagar",
      "kohima",
      "pelling",
      "shillong",
      "silchar",
      "tawang",
      "tinsukia"
    ]
  },
  "northern": {
    "label": "Northern",
    "slug": "northern",
    "cities": [
      "agra",
      "ajmer-pushkar",
      "amritsar",
      "ayodhya",
      "baltal",
      "banihal-udhampur",
      "banihar-qazigund",
      "bikaner",
      "bir",
      "chandigarh",
      "chittorgarh",
      "dalhouisie",
      "delhi",
      "dharamshala-mcleond-gang",
      "gulmarg",
      "gurugram",
      "jaipur",
      "jaisalmer",
      "jalandhar",
      "jammu",
      "jhansi",
      "jodhpur",
      "kargil",
      "katra",
      "keylong",
      "kullu",
      "kumbhalgarh",
      "leh",
      "ludhiana",
      "lukhnow",
      "manali",
      "mathura",
      "noida",
      "pathankot",
      "prayagraj",
      "sam",
      "sonmarg",
      "srinagar",
      "udaipur",
      "varanasi"
    ]
  },
  "southern": {
    "label": "Southern",
    "slug": "southern",
    "cities": [
      "allepey",
      "araku",
      "bengaluru",
      "chennai",
      "chikkmanglore",
      "coimbatore",
      "coorg",
      "gokarna",
      "hampi",
      "hyderabad",
      "kanchipuram",
      "kannur",
      "kanyakumari",
      "kochi",
      "kodaikannal",
      "kozhikode",
      "madurai",
      "mahabalipuram",
      "manglore",
      "mudurai",
      "munnar",
      "mysore",
      "ooty",
      "palakkad",
      "pondicherry",
      "poombarai",
      "rameshwaram",
      "shivamogga",
      "srisailam",
      "thanjavur",
      "theni",
      "thrissur",
      "tiruchirappalli",
      "tirupati",
      "trivandrum",
      "udupi",
      "vagamon",
      "varkala",
      "vishakhapatnam",
      "wayanad",
      "wayanad-vyithri"
    ]
  },
  "western": {
    "label": "Western",
    "slug": "western",
    "cities": [
      "ahmedabad-gandhinagar",
      "bhuj",
      "dwarka",
      "goa",
      "kevadiya",
      "kutchh",
      "mumbai",
      "nagpur",
      "nashik",
      "pune",
      "rajkot",
      "sambhajinagar",
      "shirdi",
      "somnath",
      "surat",
      "vadodara"
    ]
  }
};

export const STATES = {
  "andhra_pradesh": {
    "label": "Andhra Pradesh",
    "slug": "andhra-pradesh",
    "cities": [
      "araku",
      "srisailam",
      "tirupati",
      "vishakhapatnam"
    ]
  },
  "arunachal_pradesh": {
    "label": "Arunachal Pradesh",
    "slug": "arunachal-pradesh",
    "cities": [
      "itanagar",
      "tawang"
    ]
  },
  "assam": {
    "label": "Assam",
    "slug": "assam",
    "cities": [
      "guwahati",
      "silchar",
      "tinsukia"
    ]
  },
  "bihar": {
    "label": "Bihar",
    "slug": "bihar",
    "cities": [
      "bodh-gaya",
      "patna"
    ]
  },
  "chandigarh": {
    "label": "Chandigarh",
    "slug": "chandigarh",
    "cities": [
      "chandigarh"
    ]
  },
  "delhi": {
    "label": "Delhi",
    "slug": "delhi",
    "cities": [
      "delhi"
    ]
  },
  "goa": {
    "label": "Goa",
    "slug": "goa",
    "cities": [
      "goa"
    ]
  },
  "gujarat": {
    "label": "Gujarat",
    "slug": "gujarat",
    "cities": [
      "ahmedabad-gandhinagar",
      "bhuj",
      "dwarka",
      "kevadiya",
      "kutchh",
      "rajkot",
      "somnath",
      "surat",
      "vadodara"
    ]
  },
  "haryana": {
    "label": "Haryana",
    "slug": "haryana",
    "cities": [
      "gurugram"
    ]
  },
  "himachal_pradesh": {
    "label": "Himachal Pradesh",
    "slug": "himachal-pradesh",
    "cities": [
      "bir",
      "dalhouisie",
      "dharamshala-mcleond-gang",
      "keylong",
      "kullu",
      "manali"
    ]
  },
  "jammu_kashmir": {
    "label": "Jammu & Kashmir",
    "slug": "jammu-kashmir",
    "cities": [
      "baltal",
      "banihal-udhampur",
      "banihar-qazigund",
      "gulmarg",
      "jammu",
      "katra",
      "sonmarg",
      "srinagar"
    ]
  },
  "jharkhand": {
    "label": "Jharkhand",
    "slug": "jharkhand",
    "cities": [
      "deoghar"
    ]
  },
  "karnataka": {
    "label": "Karnataka",
    "slug": "karnataka",
    "cities": [
      "bengaluru",
      "chikkmanglore",
      "coorg",
      "gokarna",
      "hampi",
      "manglore",
      "mysore",
      "shivamogga",
      "udupi"
    ]
  },
  "kerala": {
    "label": "Kerala",
    "slug": "kerala",
    "cities": [
      "allepey",
      "kannur",
      "kochi",
      "kozhikode",
      "munnar",
      "palakkad",
      "thrissur",
      "trivandrum",
      "vagamon",
      "varkala",
      "wayanad",
      "wayanad-vyithri"
    ]
  },
  "ladakh": {
    "label": "Ladakh",
    "slug": "ladakh",
    "cities": [
      "kargil",
      "leh"
    ]
  },
  "madhya_pradesh": {
    "label": "Madhya Pradesh",
    "slug": "madhya-pradesh",
    "cities": [
      "bhopal",
      "gwalior",
      "indore",
      "khajurao",
      "omkareshwar",
      "orchha"
    ]
  },
  "maharashtra": {
    "label": "Maharashtra",
    "slug": "maharashtra",
    "cities": [
      "mumbai",
      "nagpur",
      "nashik",
      "pune",
      "sambhajinagar",
      "shirdi"
    ]
  },
  "meghalaya": {
    "label": "Meghalaya",
    "slug": "meghalaya",
    "cities": [
      "dawki",
      "shillong"
    ]
  },
  "nagaland": {
    "label": "Nagaland",
    "slug": "nagaland",
    "cities": [
      "dimapur",
      "kohima"
    ]
  },
  "odisha": {
    "label": "Odisha",
    "slug": "odisha",
    "cities": [
      "cuttak",
      "puri-konark"
    ]
  },
  "pondicherry": {
    "label": "Pondicherry",
    "slug": "pondicherry",
    "cities": [
      "pondicherry"
    ]
  },
  "punjab": {
    "label": "Punjab",
    "slug": "punjab",
    "cities": [
      "amritsar",
      "jalandhar",
      "ludhiana",
      "pathankot"
    ]
  },
  "rajasthan": {
    "label": "Rajasthan",
    "slug": "rajasthan",
    "cities": [
      "ajmer-pushkar",
      "bikaner",
      "chittorgarh",
      "jaipur",
      "jaisalmer",
      "jodhpur",
      "kumbhalgarh",
      "sam",
      "udaipur"
    ]
  },
  "sikkim": {
    "label": "Sikkim",
    "slug": "sikkim",
    "cities": [
      "gangtok",
      "pelling"
    ]
  },
  "tamil_nadu": {
    "label": "Tamil Nadu",
    "slug": "tamil-nadu",
    "cities": [
      "chennai",
      "coimbatore",
      "kanyakumari",
      "kanchipuram",
      "kodaikannal",
      "madurai",
      "mahabalipuram",
      "mudurai",
      "ooty",
      "poombarai",
      "rameshwaram",
      "thanjavur",
      "theni",
      "tiruchirappalli"
    ]
  },
  "telangana": {
    "label": "Telangana",
    "slug": "telangana",
    "cities": [
      "hyderabad"
    ]
  },
  "uttar_pradesh": {
    "label": "Uttar Pradesh",
    "slug": "uttar-pradesh",
    "cities": [
      "agra",
      "ayodhya",
      "jhansi",
      "lukhnow",
      "mathura",
      "noida",
      "prayagraj",
      "varanasi"
    ]
  },
  "west_bengal": {
    "label": "West Bengal",
    "slug": "west-bengal",
    "cities": [
      "darjeeling",
      "digha",
      "ghoom",
      "kalimpong",
      "kharagpur",
      "kolkata",
      "kurseong",
      "mayapur",
      "mirik",
      "siliguri"
    ]
  }
};

export const TRIPS = RAW_TRIPS.map(trip => ({
  ...trip,
  cities: trip.cities.map(city => ({
    ...CITIES[city.code],
    code: city.code,
    dur: city.dur,
  })),
}));

export default TRIPS;

export const BACKPACKING_101 = [
  {
    title: "Stuff you will need",
    tips: [
      "100L rucksack bag + 20-30L foldable bags",
      "Packing cubes or pouches to separate clothes, cables, and toiletries.",
      "Basic medicines, reusable water bottle, and a rain cover.",
      "Power bank, 4m usb-c cable, wired earphones",
      "A small padlock for lockers, and a cheap but sturdy money belt.",
    ],
  },
  {
    title: "Stay",
    tips: [
      "Book the first night in advance — never arrive in a new city without a confirmed bed.",
      "Hostels for social energy, guesthouses for quiet — pick based on what you need that week.",
      "Check checkout time before sleeping. Delayed checkouts are usually easy to negotiate the night before.",
      "Keep the property address saved offline and in the local language.",
    ],
  },
  {
    title: "Travel",
    tips: [
      "Overnight trains between cities save a night's accommodation and travel time.",
      "Book sleeper class early — side lower fills fast on popular routes.",
      "Carry snacks, a water bottle, and a light jacket for long journeys.",
      "Keep one accessible fresh set of clothes for delayed arrivals or missed connections.",
    ],
  },
  {
    title: "Budgeting",
    tips: [
      "Split cash across wallet, backpack, and phone cover — never keep it all together.",
      "Small notes matter: buses, autos, tea stalls, and cloakrooms rarely give change.",
      "Keep one backup card physically separate from your daily card.",
      "Track spending city by city — patterns show where money leaks.",
    ],
  },
  {
    title: "Hacks",
    tips: [
      "Do laundry the day before you move, not the day you arrive tired.",
      "A sleep kit for trains — earplugs, eye mask, light blanket — makes overnight travel actually restful.",
      "Extension board for awkward plug points in hostels, hotels, and cafes.",
      "Save offline maps, tickets, and bookings before any travel day.",
    ],
  },
  {
    title: "Red Zone",
    tips: [
      "Never share your location or travel plan with someone you just met.",
      "Avoid unlicensed taxis at night — use apps or ask the hotel to arrange.",
      "If a place feels off, leave. Trust that instinct without needing to explain it.",
      "Keep emergency contacts and a copy of your ID in a separate bag.",
    ],
  },
];
